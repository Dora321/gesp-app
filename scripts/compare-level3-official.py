...
LOCAL_DIR = ROOT / "src" / "data" / "gesp" / "level3"
OFFICIAL_DIR = ROOT / "tmp" / "official" / "l3"
REPORT_JSON = ROOT / "tmp" / "level3_official_compare.json"
REPORT_MD = ROOT / "tmp" / "level3_official_compare.md"
...
def extract_local(file_path: Path) -> dict:
    ...
    single_answers = "".join(chr(ord("A") + q["answer"]) for q in paper["questions"] if q["type"] == "single")
    judge_answers = "".join("T" if q["answer"] == 0 else "F" for q in paper["questions"] if q["type"] == "judge")
    programming_titles = []
    for q in paper.get("programmingQuestions", []):
        title = q.get("title") or parse_local_title(q.get("question", ""))
        programming_titles.append(title)
    objective_questions = [q["question"] for q in paper["questions"] if q["type"] in {"single", "judge"}]
    return {
        "id": paper["id"],
        "title": paper["title"],
...
def extract_analysis_style(text: str, lines: list[str]) -> OfficialPaper | None:
    if "真题解析" not in text:
        return None

    single_answers = "".join(re.findall(r"【答案】([A-D])", text)[:15])
    single_questions: list[str] = []
    judge_questions: list[str] = []
    programming_titles: list[str] = []
    ...
    mode = None
    current: list[str] = []
    in_programming_title_zone = False
    for line in lines:
        stripped = line.strip()
        if not stripped:
            continue
        if "一、单选题" in stripped:
            mode = "single"
            current = []
            continue
        if "二、判断题" in stripped:
            if current:
                single_questions.append(" ".join(current))
                current = []
            mode = "judge"
            continue
        if "三、编程题" in stripped:
            if current:
                judge_questions.append(" ".join(current))
                current = []
            mode = "programming"
            in_programming_title_zone = True
            continue

        if mode in {"single", "judge"} and re.match(r"^\d+\.\s+", stripped):
            if current:
                if mode == "single":
                    single_questions.append(" ".join(current))
                else:
        ...
        if mode == "programming" and "【问题描述】" in stripped:
            in_programming_title_zone = False
            continue

        if mode == "programming" and in_programming_title_zone and re.match(r"^[12]\.\s+", stripped):
            programming_titles.append(re.sub(r"^[12]\.\s+", "", stripped))
            continue

        if mode in {"single", "judge"} and current and not stripped.startswith("【"):
            if not re.match(r"^[A-D]\.", stripped):
    ...
    return OfficialPaper(
        single_answers=single_answers,
        single_questions=single_questions[:15],
        judge_questions=judge_questions[:10],
        programming_titles=programming_titles[:2],
    )
...
def extract_official(file_path: Path) -> OfficialPaper:
    ...
    analysis_style = extract_analysis_style(text, lines)
    if analysis_style is not None:
        return analysis_style

    single_answers = ""
    if len(lines) >= 2:
        letters = re.findall(r"\b([A-D])\b", lines[1])
...
def main() -> int:
    local_files = sorted(p for p in LOCAL_DIR.glob("*.js") if re.match(r"\d{4}-\d{2}-l3\.js$", p.name))
    reports = []
    for local_file in local_files:
        if local_file.name == "2023-03-l3.js":
            continue
        ...
        single_matches = local["single_answers"] == official.single_answers

        local_single_questions = local["objective_questions"][:15]
        local_judge_questions = local["objective_questions"][15:25]
        single_scores = [
            round(similarity(a, b), 4)
            for a, b in zip(local_single_questions, official.single_questions)
        ]
        judge_scores = [
            round(similarity(a, b), 4)
            for a, b in zip(local_judge_questions, official.judge_questions)
        ]
        title_scores = [
            round(similarity(a, b), 4)
        ...
        reports.append(
            {
                ...
                "single_answers_match": single_matches,
                ...
                "single_question_scores": single_scores,
                "judge_question_scores": judge_scores,
            }
        )

    REPORT_JSON.write_text(json.dumps(reports, ensure_ascii=False, indent=2), encoding="utf-8")

    lines = ["# Level 3 Official Comparison", ""]
    for item in reports:
        if "error" in item:
