...
const coreRoutes = [
  ...
  '/hardware', '/hardware/esp32-curriculum', '/innovation-foundation',
...
async function main() {
  ...
  const { foundationLessons, innovationLessons } = await import('../src/data/innovationCourseCatalog.js');
  const routes = [...new Set([...coreRoutes, ...cppLessonPaths, ...[...foundationLessons, ...innovationLessons].map(lesson => lesson.path), ...paperEntries])];
