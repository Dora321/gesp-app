 ...
 *
 * Scans src/data/gesp/level{1-8}/ and writes src/data/gesp/_generated.js
 * which is then imported by src/data/gesp/index.js
 */
...
const paperMetaLines = papers.map(p =>
  `  '${p.id}': { level: ${p.level}, year: ${p.year}, month: ${p.month}, title: '${p.title}', questionCount: ${p.questionCount} },`
).join('\n');
...
console.log(`Total question count across all papers: ${papers.reduce((sum, p) => sum + p.questionCount, 0)}`);