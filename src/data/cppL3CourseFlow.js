...
const qualityByLesson = {
  ...
  8: {
    accent: 'rose',
    goals: ['能统计数字、字母、空格等字符类型', '能按字符范围转换大小写', '能使用 find、substr 和 replace 处理子串'],
    deliverables: ['完成一个字符分类统计程序', '写出大小写转换和关键词查找两个小工具', '用 find 定位后分别练习截取、首次替换和未找到时保持原串'],
    checks: ['能用字符范围判断数字、字母和空格', '能在转换大小写前先判断目标字符范围', '能用 string::npos 判断 find 没有找到，且只在找到时调用 replace'],
  },
  9: {
    accent: 'rose',
...
const reviewTasksByLesson = {
  ...
  8: ['统计字符串中的数字、字母和空格数量。', '用 find 找关键词，找到后只替换首次匹配；未找到则原样输出。'],
