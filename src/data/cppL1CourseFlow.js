...
const qualityByLesson = {
  ...
  2: {
    accent: 'blue',
    goals: ['理解 int 变量的定义、初始化与再次赋值', '能区分变量和 const 常量', '掌握 C++ 变量命名的基本规则', '能用 cin 读入整数并用 cout 输出计算结果'],
    deliverables: ['完成一组合法/非法变量名分类题', '写出使用单价常量、读入两个数量并计算文具总价的小程序', '整理变量与常量对比表'],
    checks: ['能解释为什么变量名不能有空格和减号', '能说明 const 常量为什么不能再次赋值', '能说明 cin >> a 把输入读入变量 a', '能发现未声明变量和关键字冲突问题'],
  },
  3: {
    accent: 'blue',
  ...
  6: {
    accent: 'blue',
    goals: ['理解 &&、||、! 三种逻辑运算', '掌握非零即真与 &&、|| 的短路求值', '能避免链式比较和优先级陷阱'],
    deliverables: ['完成逻辑门模拟实验', '写出一个门票/权限判断程序', '整理链式比较与短路求值易错清单'],
    checks: ['能预测短路时右侧是否执行', '能把生活规则翻译成 && 和 ||', '能指出 1 < x < 10 为什么不可靠'],
  },
  7: {
    accent: 'blue',
  ...
  8: {
    accent: 'blue',
    goals: ['理解 if / else if / else 的多重选择顺序', '能判断条件从上到下只会命中第一个分支', '会用 switch 处理固定选项并解释 break'],
    deliverables: ['完成智能餐厅多分支模拟', '写出成绩等级判断程序', '完成一个带 default 的菜单选择程序'],
    checks: ['能预测多个条件同时满足时命中哪一支', '能解释 else if 顺序为什么重要', '能预测 switch 漏写 break 的输出'],
  },
  9: {
    accent: 'blue',
    ...
    checks: ['能说清 for 括号内由两个分号分隔的初始化、条件和更新三段', '能手算循环最后一次执行后的 i 值', '能识别漏写更新语句造成的死循环'],
  },
  10: {
    accent: 'blue',
    goals: ['理解 while 先判断与 do-while 后判断的差异', '掌握数位分离中的 / 与 % 组合', '能区分 for 与 while 的适用场景'],
    deliverables: ['写出一个整数各位数字求和程序', '画出 while 条件变化表', '手推初始条件为假时 while 与 do-while 的不同输出'],
    checks: ['能说明循环变量如何变化', '能判断 while(1) 何时需要 break', '能解释 do-while 至少执行一次且结尾需要分号'],
  },
  11: {
    accent: 'indigo',
