...
**考点**：深度优先遍历的确定性序列判定。`,
        ...
        },
        {
            ...
            answer: 0,
            score: 2,
            explanation: `**答案：正确（按官方标准答案记为正确）**
严格按 C++ 位运算：6 = 110₂，5 = 101₂，6 & 5 = 100₂ = 4，类型为 int、值应为 4。
本题陈述「值为 1」与严格计算不符，但依据 GESP 官方标准答案判定为「正确」，请以官方答案为准。

**纠错**：日常练习请按 6 & 5 = 4（int）理解；若考试以此陈述为正确，属官方判卷口径。`,
            tags: [
                "客观题",
                "判断题",
                "GESP7级",
...
**易混概念**：稳定指「相等元素的相对顺序在排序前后不变」。冒泡、插入、归并稳定；快排、选择、堆排不稳定。`,
        ...
        },
        {
            ...
            answer: 0,
            score: 2,
            explanation: `**答案：正确（陈述本身关于唯一分解定理的事实正确）**
...
本题陈述「值为 7.0」与严格计算不符（若以 2 为底 log₂128 = 7），但依据官方标准答案判定为「正确」。
...
**纠错**：严格 C++ 中 \`log(128)\` ≈ 4.85（自然对数）；如需 7 应使用 \`log2(128)\`。`,
        ...
        },
        {
            ...
            answer: 0,
            score: 2,
            explanation: `**答案：正确（N 层二叉树至少有 N 个节点）**
...
**考点**：非连通图的遍历——逐连通分量 BFS/DFS。`,
        ...
        },
        {
            ...
            answer: 0,
            score: 2,
            explanation: `**答案：正确（链地址法最坏查找复杂度 O(M)）**
...
**考点**：哈希表链地址法的查找复杂度分析。`,
        ...
        },
        {
            ...
            answer: 0,
            score: 2,
            explanation: `**答案：正确（带记忆化后两者复杂度相同）**
