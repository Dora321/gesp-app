...
按图（A 为根，左子树 B-D、右子树 C-E 的典型结构）的各遍历：
- 先序（根-左-右）：A B D C E，故 A 错。
- 中序（左-根-右）：B D A E C，故 B 错。
- 后序（左-右-根）：D B E C A，故 C 错。
- 广度优先（按层，从上到下、同层从左到右）：A B C D E，故 D 正确。

**注**：源文件未包含该二叉树配图，以上按标准答案 D 对应的“BFS 序列为 ABCDE”反推各遍历结果。

**考点**：二叉树的先序/中序/后序/层序（广度优先）遍历。`,
            tags: [
                "客观题",
                "单选题",
                "GESP7级",
