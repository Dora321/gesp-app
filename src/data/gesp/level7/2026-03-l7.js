...
**解析：** 检验每个选项是否为三序列的公共子序列（保持相对先后顺序）：
- A \`{1,8,5,6}\`：s1 中 1,8,5,6（位置 1,2,4,5）✓；s2 中 1,8,5,6（0,3,7,8）✓；s3 中 1,8,5,6（0,1,2,5）✓ → 是公共子序列，长度 4。
- B \`{1,5,6,7}\`：s3 中 7 在位置 4、6 在位置 5，要求 6 出现在 7 之前，顺序不符 ✗。
- C \`{1,8,6}\`：是公共子序列，但长度仅 3，不是最长。
- D \`{1,5,7,4}\`：s2 中 7 在位置 6、4 在位置 5，要求 7 在 4 之前，顺序不符 ✗。

故 A 为最长公共子序列（长度 4）。

**考点**：最长公共子序列（LCS）的定义与判定。`,
            tags: [
                "客观题",
                "单选题",
                "GESP7级",
        ...
        },
        {
            ...
            answer: 3,
            score: 2,
            explanation: `**答案：D（地址 3）**

**解析：** 开放定址法线性探测，\`H(key)=key mod 7\`，地址区间 \`[0, n-1]\`（n 足够大）。依次插入 \`{20,30,20,15,6}\`：
- 20 → 20%7=6，地址 6 空，存入 6。
- 30 → 30%7=2，地址 2 空，存入 2。
- 20 → 6 已占，探测 0 空，存入 0。
- 15 → 15%7=1，地址 1 空，存入 1。
- 6 → 6 占→0 占→1 占→2 占→3 空，存入地址 3。

故元素 6 存储在地址 3。

**考点**：哈希表开放定址法（线性探测）与冲突处理。`,
            tags: [
                "客观题",
                "单选题",
                "GESP7级",
...
**解析：** 完全二叉树深度为 6（根深度为 1），即前 5 层必须满、第 6 层从左到右连续填充。节点最少时第 6 层只有 1 个节点：前 5 层满共 \`2^5-1 = 31\` 个，加第 6 层 1 个，共 32。64 是深度 6 满二叉树的节点数，不是“最少”。
...
**解析：** 哈夫曼合并（频次 2,2,3,3,5）：
- 合并最小两堆 2+2=4 → {3,3,4,5}
- 3+3=6 → {4,5,6}
- 4+5=9 → {6,9}
- 6+9=15

WPL = 所有内部结点权值之和 = 4+6+9+15 = 34。也可按带权路径：2、2 在深度 3（各 6），3、3 在深度 2（各 6），5 在深度 2（10），合计 6+6+6+6+10 = 34。
...
**解析：** 握手定理：无向图所有顶点度数之和 = 2 × 边数 = 2 × 36 = 72。每个顶点度数均为 4，则顶点数 = 72 / 4 = 18。
...
**解析：** 主定理（Master Theorem）：\`T(n)=2T(n/2)+n\`，\`a=2, b=2, f(n)=n = Θ(n^{log_b a}) = Θ(n)\`，属 case 2，故 \`T(n)=Θ(n log n)\`。注意不是 \`O(log n)\`（那是递归树高度，未乘每层代价）。
