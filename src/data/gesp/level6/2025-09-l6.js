    ...
    questions: [
        {
            ...
            **解析：**
            ...
            - **A 构造函数不能声明为虚函数，但析构函数可以。**：错误。
            - **B 函数参数如声明为类的引用类型，调用时不会调用该类的复制构造函数。**：错误。
            - **C 静态方法属于类而不是某个具体对象，因此推荐用类名::方法(...)调用。**：错误。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **B 封装 (Encapsulation)**：错误。
            - **C 多态 (Polymorphism)**：错误。
            - **D 链接 (Linking)**：错误。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **B 完全二叉树的叶子节点可以出现在最后一层的任意位置**：错误。
            - **C 所有节点都有两个孩子**：错误。
            - **D 左孩子位于 2i，右孩子位于 2i+1**：错误。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A a: 00；b: 01；c: 10；d: 110；e: 111；f: 0**：错误。
            - **B a: 1100；b: 1101；c: 100；d: 101；e: 111；f: ...**：正确答案。
            - **C a: 000；b: 001；c: 01；d: 10；e: 110；f: 111**：错误。
            - **D a: 10；b: 01；c: 100；d: 101；e: 111；f: 0**：错误。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A int i = 0; i < prev.size(); i++**：错误。该代码逻辑与题目要求不符，请逐步推演。
            - **B int i = prev.size()-1; i >= 0; i--**：正确答案。
            - **C auto s : prev**：错误。
            - **D int i = prev.size()/2; i < prev.size(); ...**：错误。该代码逻辑与题目要求不符，请逐步推演。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A vector**：错误。
            - **B list**：错误。
            - **C queue**：错误。对队列的先进先出特性或循环队列满判断理解有误。
            - **D stack**：正确答案。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A dp[w] = max(dp[w], dp[w]+val[i]);**：错误。动态规划的状态或转移方程有误，请检查边界初始化。
            - **B dp[w] = dp[w-wt[i]]+val[i];**：错误。动态规划的状态或转移方程有误，请检查边界初始化。
            - **C dp[w] = max(dp[w-1], dp[w-wt[i]]+val[i])...**：错误。动态规划的状态或转移方程有误，请检查边界初始化。
