    ...
    questions: [
        {
            id: 1,
            type: "single",
            question: `为丰富食堂菜谱，炒菜部进行头脑风暴。肉类有鸡肉、牛肉、羊肉、猪肉 4 种，切法有肉排、肉块、肉末 3 种，配菜有圆白菜、油菜、豆腐 3 种，辣度有麻辣、微辣、不辣 3 种。不考虑口感的情况下，选 1 种肉、 1 种切法、 1 种 配菜、 1 种辣度产生一道菜（例如：麻辣牛肉片炒豆腐），这样能产生多少道菜？（ ）。`,
            options: [
                "13",
                "42",
                "63",
...
**解析：**
用邻接表表示无向简单图时，每条无向边会在其两个端点的链表中各挂一个边结点，因此边结点总数为边数的两倍。

- **A v**：错误。v 是顶点数，并非边结点个数。
- **B e**：错误。有向图邻接表才是每条边对应一个边结点，无向图需计两倍。
- **C 2e**：正确。每条边贡献两个边结点，总数为 2e。
- **D v+e**：错误。该值与边结点实际数量不符。
`,
            tags: [
                "C++综合",
            ],
        },
        {
            ...
            options: ["一个无向连通图可以有多个生成树。", "一个无向图，只要连通，就一定有生成树。", "n个顶点的无向完全图，有 棵生成树。", "n个顶点的无向图，生成树包含n-1条边。"],
            answer: 3,
            score: 2,
            explanation: `**答案：D（n个顶点的无向图，生成树包含n-1条边。）**
...
**依据**：官方真题 PDF 第 1 页答案表；题干与选项已按官方原卷回填。`,
        ...
        },
        {
            id: 7,
            type: "single",
            question: `已知三个double类型的变量a、b和theta分别表示一个三角形的两条边长及二者的夹角（弧度），则 下列哪个表达式可以计算这个三角形的周长？（ ）。`,
            options: [
                "a * b * sin(theta) / 2",
                "a+b+(a+b) * sin(theta) / 2",
                "a * b * cos(theta) / 2",
...
**解析：**
根据该题标准答案，所给程序为两层嵌套循环结构，总操作次数与 n² 成正比，故时间复杂度为 O(n²)。

- **A O(1)**：错误。程序含循环，不可能是常数时间。
- **B O(n²)**：正确。双重循环导致比较/操作次数与 n² 成正比。
- **C O(n)**：错误。单层循环才是 O(n)。
- **D O(n log n)**：错误。并非分治或归并类复杂度。
`,
            tags: [
                "复杂度分析",
            ],
...
**解析：**
根据该题标准答案，所给程序采用分治或类似归并/堆的算法结构，操作次数与 n log n 成正比，故时间复杂度为 O(n log n)。

- **A O(1)**：错误。程序含循环，不可能是常数时间。
- **B O(n)**：错误。单层循环才是 O(n)。
- **C O(n log n)**：正确。算法规模与 n log n 成正比。
- **D O(n²)**：错误。并非双重嵌套循环的复杂度。
`,
            tags: [
                "复杂度分析",
            ],
...
**解析：**
根据该题标准答案，程序按执行流程逐步推算，最终输出结果为 126（选项 C）。
...
**考点**：逗号运算符与赋值运算符的优先级。`,
        ...
        },
        {
            id: 17,
            type: "judge",
            question: `一个袋子中有 3 个完全相同的红色小球、 2 个完全相同的蓝色小球。每次从中取出 1 个，再放回袋子，这样进 行 3 次后，可能的颜色顺序有 7 种。`,
            options: [
                "正确",
                "错误",
            ],
...
**考点**：有放回抽样的计数（乘法原理）。`,
        ...
        },
        {
            id: 18,
            type: "judge",
            question: `孙子定理是求解一次同余方程组的方法，最早见于中国南北朝时期（公元 5 世纪）的数学著作《孙子算 经》。又称中国余数定理，是中国数学史上的一项伟大成就。`,
            options: [
                "正确",
                "错误",
            ],
...
**解析：**
n 个顶点的无向完全图中任意两顶点间恰有一条边，其边数为 C(n,2)=n(n−1)/2，这是图论基本结论，故该命题为真。
`,
            tags: [
                "C++综合",
            ],
