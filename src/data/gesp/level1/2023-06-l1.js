  ...
  questions: [
    // 1-15 单选题
    {
      ...
      **解析：**
      ...
      - **B 打印机**：错误。此说法不正确。麦克风用于输入声音信号，属于输入设备。
      - **C 显示器**：错误。此说法不正确。麦克风用于输入声音信号，属于输入设备。
      - **D 音箱**：错误。此说法不正确。麦克风用于输入声音信号，属于输入设备。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 编写程序**：错误。此说法不正确。ChatGPT 是数字世界的语言模型，无法在物理世界中进行烹饪等体力劳动。
      - **B 翻译语言**：错误。此说法不正确。ChatGPT 是数字世界的语言模型，无法在物理世界中进行烹饪等体力劳动。
      - **C 炒菜**：正确。ChatGPT 是数字世界的语言模型，无法在物理世界中进行烹饪等体力劳动。
      - **D 写诗**：错误。此说法不正确。ChatGPT 是数字世界的语言模型，无法在物理世界中进行烹饪等体力劳动。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **B 2023_GESP**：错误。此说法不正确。B以数字开头，C包含减号，D是标准库对象名（虽可作为变量名但不推荐，但A显然更符合常规标识符定义）。
      - **C GESP-2023**：错误。此说法不正确。B以数字开头，C包含减号，D是标准库对象名（虽可作为变量名但不推荐，但A显然更符合常规标识符定义）。
      - **D cout**：错误。代码逻辑有误。B以数字开头，C包含减号，D是标准库对象名（虽可作为变量名但不推荐，但A显然更符合常规标识符定义）。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A int**：错误。此说法不正确。endl 是在 iostream 中定义的操纵符，不是语言本身保留的关键字。
      - **B for**：错误。代码逻辑有误。endl 是在 iostream 中定义的操纵符，不是语言本身保留的关键字。
      - **C if**：错误。代码逻辑有误。endl 是在 iostream 中定义的操纵符，不是语言本身保留的关键字。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A a = b+c;**：错误。此说法不正确。赋值运算符的左边必须是一个可以修改的左值（lvalue），表达式 a+c 不能作为左值。
      - **B b = a;**：错误。此说法不正确。赋值运算符的左边必须是一个可以修改的左值（lvalue），表达式 a+c 不能作为左值。
      - **C c = a / b;**：错误。此说法不正确。赋值运算符的左边必须是一个可以修改的左值（lvalue），表达式 a+c 不能作为左值。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **B pow(a, 2)**：错误。此说法不正确。在 C++ 中，^ 是按位异或运算符，不是乘方。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **B 22.75**：错误。计算有误。4 * 23 / 4 = 92 / 4 = 23。
      - **C 23**：正确。4 * 23 / 4 = 92 / 4 = 23。
      - **D 25**：错误。计算有误。4 * 23 / 4 = 92 / 4 = 23。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **C a+b == 0**：错误。代码逻辑有误。逻辑与判定。注意 !(a || b) 虽然逻辑等价（当a,b为bool时），但 D 最符合直觉。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A i % 2 == 1**：错误。代码逻辑有误。1 % 3 = 1, 4 % 3 = 1, 7 % 3 = 1。
      - **B i % 3 == 1**：正确。1 % 3 = 1, 4 % 3 = 1, 7 % 3 = 1。
      - **C i % 4 == 1**：错误。代码逻辑有误。1 % 3 = 1, 4 % 3 = 1, 7 % 3 = 1。
      - **D i % 7 == 1**：错误。代码逻辑有误。1 % 3 = 1, 4 % 3 = 1, 7 % 3 = 1。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 55**：错误。计算有误。局部变量未被初始化，其值是随机的。
      - **B 0**：错误。计算有误。局部变量未被初始化，其值是随机的。
      - **C 45**：错误。计算有误。局部变量未被初始化，其值是随机的。
