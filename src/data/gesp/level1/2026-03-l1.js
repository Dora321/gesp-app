  ...
  questions: [
    {
      ...
      **解析：**
      算法程序属于程序/软件，不是输入设备；其余三项都可用于采集信息。
    ...
    },
    {
      ...
      **解析：**
      在常见图形界面中，选中文件后右键选择“重命名”即可进入编辑文件名状态。
      
      - **A 单击右键并选择弹出菜单中的“重命名”**：正确答案。
      - **B 双击左键**：错误。
      - **C 按功能键 F1**：错误。
      - **D 按回车键**：错误。
    ...
    },
    {
      ...
      **解析：**
      C++ 标识符区分大小写；只要定义和使用保持一致，变量名改成 \`Pai\` 也可以正常执行。
      
      - **A 为了方便初学者，\`cout << (PI)\` 和 \`cout << (pi)\` ...**：错误。
      - **B 把 \`cout << (PI)\` 修改为 \`cout << (Pi)\` 能正常执...**：错误。
      - **C 不能用 \`PI\` 做变量名，因为要保存圆周率这个常量**：错误。
      - **D 将程序中全部 \`PI\` 都改写为 \`Pai\`，将能正常执行，不会报错**：正确答案。
    ...
    },
    {
      ...
      **解析：**
      逗号表达式最终使 \`a=4\`、\`b=4\`。第一行输出 \`6\` 和 \`2\`，第二行输出 \`4\` 和 \`4\`。官方答案为 A，对应题面版式显示为 \`61 / 43\`。
    ...
    },
    {
      ...
      **解析：**
      把整数 \`10\` 输入给整型变量 \`N\` 后会正常输出 \`10\`。
      
      - **A 执行时如输入 \`10\`，则将输出 \`10\`**：正确答案。
      - **B 执行时如输入 \`3.14\`，将报错**：错误。
      - **C 执行时如输入 \`ABC\`，将报错**：错误。
      - **D 执行时如输入 \`-10\`，将报错**：错误。
    ...
    },
    {
      ...
      **解析：**
      这段程序输出的是两个整数之差的绝对值，因此结果总是大于等于 0。
      
      - **A 如果输入一个正数和一个负数，其输出结果肯定是大于 0**：错误。
      - **B 不管是负整数、正整数亦或 0，其结果肯定是大于等于 0**：正确答案。
      - **C 如果 \`N\` 和 \`M\` 是相等的整数，将不会有输出**：错误。
      - **D 如果 \`N\` 和 \`M\` 输入带有小数点的数，将按整数部分计算**：错误。
    ...
    },
    {
      ...
      **解析：**
      \`i=0\` 时 \`tnt*=0\` 之后就一直为 0；循环结束时 \`i=5\`，因此输出 \`0 5\`。
    ...
    },
    {
      ...
      **解析：**
      每三个数中第 1 个取负，其余两个取正，和题目给出的规律一致。
      
      - **A L1 行中 \`i < N + 1\` 应该改为 \`i < N\` 才会符合预期**：错误。
      - **B L2 行中 \`i % 3 == 1\` 应修改为 \`i % 3 == 0\` 才会符...**：错误。
      - **C L2 行中 \`i % 3 == 1\` 修改为 \`i % 3\` 与当前程序效果相同**：错误。
      - **D 当前代码能实现题目所描述计算目标**：正确答案。
    ...
    },
    {
      ...
      **解析：**
      \`continue\` 会直接跳过偶数时后面的输出，因此删除 \`else\` 不影响奇数输出逻辑。
      
      - **A 上述代码执行后，其输出是 \`1#3#5#7#9#9END\`**：错误。
      - **B 删除 \`else\` 后的执行效果与当前代码相同**：正确答案。
      - **C 删除 \`else\` 且将 \`cout << i << "#"\` 移入 L1 行下...**：错误。
      - **D 在 \`cout << i << "END"\` 前增加判断 \`if(i > 10)...**：错误。
    ...
    },
    {
      ...
      **解析：**
      每次取个位 \`N % 10\` 加入和中，再用 \`N /= 10\` 去掉当前个位。
    ...
    },
    {
      ...
      **解析：**
      当前代码本身可以正确判断；而变量 \`Q\` 只是中间量，也可以在分支里直接构造并判断。
      
      - **A 代码段不能完成正确判断**：错误。
      - **B L1 行代码中条件应该改为 \`M <= N\`**：错误。
      - **C L2 行代码应该改为 \`if ((Q*10+Q/3) % 14 == 0)\`**：错误。
      - **D 代码段可以不使用变量 \`Q\`，而是在区分 \`M\` 和 \`N\` 大小后分别直接用它...**：正确答案。
    ...
    },
    {
      ...
      **解析：**
      高 3 位为 \`N / 1000\`，低 3 位为 \`N % 1000\`，判断是否为 314 的倍数应用 \`% 314 == 0\`。
      
      - **A ((N % 1000) - (N / 1000)) / 314 == 0**：错误。
      - **B ((N / 1000) - (N % 1000)) % 314 == 0**：正确答案。
      - **C ((N % 1000) - (N / 1000)) / 314**：错误。
      - **D ((N / 1000) - (N % 1000)) % 314**：错误。
    ...
    },
    {
      ...
      **解析：**
      要交替取个位累加到 \`N1\` 和 \`N2\`，每次都需要用 \`N /= 10\` 去掉已处理的最后一位。
    ...
    },
    {
      ...
      **判定依据：**
      循环会在 \`i=3\` 时跳出，最终输出 \`3\`，不是 \`0\`。
      
      **纠错：** 原命题说法有误。循环会在 \`i=3\` 时跳出，最终输出 \`3\`，不是 \`0\`。
    ...
    },
    {
      ...
      **判定依据：**
      \`for\` 后面多了一个分号，导致循环体为空，不能正确求和。
      
      **纠错：** 原命题说法有误。\`for\` 后面多了一个分号，导致循环体为空，不能正确求和。
    ...
    },
    {
      ...
      **判定依据：**
      若输入 \`0\`，循环体执行 0 次；若输入非 0 且循环内不修改 \`N\`，就会无限输出。
    ...
    },
    {
      ...
      **判定依据：**
      该等差数列从 \`-N\` 开始，每次加 2，到小于 \`N\` 为止，和恒为 \`-N\`。
    ...
    },
    {
      ...
      **判定依据：**
      这属于格式与实参类型不匹配，通常会产生未定义行为，但不一定在编译期直接报错。
      
      **纠错：** 原命题说法有误。这属于格式与实参类型不匹配，通常会产生未定义行为，但不一定在编译期直接报错。
    ...
    },
    {
      ...
      **判定依据：**
      循环中每次相当于把奇数 \`1,3,5,...,99\` 加到 \`cnt\`，前 50 个奇数和为 \`2500\`。
    ...
    },
    {
      ...
      **判定依据：**
      \`%-5d\` 表示左对齐、宽度至少为 5，输出效果是 \`314  \`，不是 \`-5314\`。
      
      **纠错：** 原命题说法有误。\`%-5d\` 表示左对齐、宽度至少为 5，输出效果是 \`314  \`，不是 \`-5314\`。
