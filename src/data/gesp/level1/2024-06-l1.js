  ...
  questions: [
    {
      ...
      **解析：**
      ...
      - **A five-Star**：正确。变量名不能包含连字符 \`-\`。
      - **B five_star**：错误。此说法不正确。变量名不能包含连字符 \`-\`。
      - **C fiveStar**：错误。此说法不正确。变量名不能包含连字符 \`-\`。
      - **D _fiveStar**：错误。此说法不正确。变量名不能包含连字符 \`-\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A -1.2**：错误。计算有误。先算乘除，\`3*3=9\`，整数除法 \`9/5=1\`，再算 \`3-1=2\`。
      - **B 1**：错误。可能差一错误（off-by-one），如循环条件用了 <= 而非 <。正确结果为 2。
      - **C 0**：错误。计算有误。先算乘除，\`3*3=9\`，整数除法 \`9/5=1\`，再算 \`3-1=2\`。
      - **D 2**：正确。先算乘除，\`3*3=9\`，整数除法 \`9/5=1\`，再算 \`3-1=2\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 6**：错误。计算有误。\`N % 3\` 最大为 2，\`N % 7\` 最大为 6，总和最大为 8。
      - **B 8**：正确。\`N % 3\` 最大为 2，\`N % 7\` 最大为 6，总和最大为 8。
      - **C 9**：错误。可能差一错误（off-by-one），如循环条件用了 <= 而非 <。正确结果为 8。
      - **D 10**：错误。计算有误。\`N % 3\` 最大为 2，\`N % 7\` 最大为 6，总和最大为 8。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 1={1}**：错误。此说法不正确。\`%%\` 输出一个 \`%\`，\`5 % 2\` 的值为 \`1\`。
      - **B 5%2={5%2}**：错误。代码逻辑有误。\`%%\` 输出一个 \`%\`，\`5 % 2\` 的值为 \`1\`。
      - **C 5%2={1}**：正确。\`%%\` 输出一个 \`%\`，\`5 % 2\` 的值为 \`1\`。
      - **D 5 ={1}**：错误。此说法不正确。\`%%\` 输出一个 \`%\`，\`5 % 2\` 的值为 \`1\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 将输出 7**：错误。此说法不正确。\`cin\` 读取整数时遇到 \`+\` 会停止，因此只读到 \`5\`。
      - **B 将输出 5**：正确。\`cin\` 读取整数时遇到 \`+\` 会停止，因此只读到 \`5\`。
      - **C 语句会报错**：错误。此说法不正确。\`cin\` 读取整数时遇到 \`+\` 会停止，因此只读到 \`5\`。
      - **D 输出不确定**：错误。此说法不正确。\`cin\` 读取整数时遇到 \`+\` 会停止，因此只读到 \`5\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 102={102}**：错误。此说法不正确。\`(int)101.1\` 得到 \`101\`，所以输出 \`a + 1 = {102}\`。
      - **B a+1={a+1}**：错误。此说法不正确。\`(int)101.1\` 得到 \`101\`，所以输出 \`a + 1 = {102}\`。
      - **C a + 1 = {102}**：正确。\`(int)101.1\` 得到 \`101\`，所以输出 \`a + 1 = {102}\`。
      - **D 会报错**：错误。此说法不正确。\`(int)101.1\` 得到 \`101\`，所以输出 \`a + 1 = {102}\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A -17.75**：错误。计算有误。\`9/4=2\`，\`6%(6-2)=2\`，所以 \`2-2*10=-18\`。
      - **B -18**：正确。\`9/4=2\`，\`6%(6-2)=2\`，所以 \`2-2*10=-18\`。
      - **C -14**：错误。计算有误。\`9/4=2\`，\`6%(6-2)=2\`，所以 \`2-2*10=-18\`。
      - **D -12.75**：错误。计算有误。\`9/4=2\`，\`6%(6-2)=2\`，所以 \`2-2*10=-18\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 第5行代码1**：错误。此说法不正确。\`10 % 3 == 1\`，进入 \`if\` 分支，\`%2d\` 会保留宽度 2，因此前面有一个空格。
      - **B 第6行代码1**：错误。此说法不正确。\`10 % 3 == 1\`，进入 \`if\` 分支，\`%2d\` 会保留宽度 2，因此前面有一个空格。
      - **C 第5行代码 1**：正确。\`10 % 3 == 1\`，进入 \`if\` 分支，\`%2d\` 会保留宽度 2，因此前面有一个空格。
      - **D 第6行代码 1**：错误。此说法不正确。\`10 % 3 == 1\`，进入 \`if\` 分支，\`%2d\` 会保留宽度 2，因此前面有一个空格。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 9 45**：错误。此说法不正确。循环结束时 \`i=10\`，\`sum\` 为 \`0+1+...+9=45\`。
      - **B 10 55**：错误。此说法不正确。循环结束时 \`i=10\`，\`sum\` 为 \`0+1+...+9=45\`。
      - **C 10 45**：正确。循环结束时 \`i=10\`，\`sum\` 为 \`0+1+...+9=45\`。
      - **D 11 55**：错误。此说法不正确。循环结束时 \`i=10\`，\`sum\` 为 \`0+1+...+9=45\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 输入负数可能输出“是质数”**：错误。此说法不正确。标准质数判定中，\`N=2\` 时循环可能一次也不执行，但它仍然是质数。
      - **B 输入 2 会输出“不是质数”**：错误。此说法不正确。标准质数判定中，\`N=2\` 时循环可能一次也不执行，但它仍然是质数。
      - **C 输入 2 会输出“是质数”，即使循环体未执行**：正确。标准质数判定中，\`N=2\` 时循环可能一次也不执行，但它仍然是质数。
      - **D 把 \`N>=2\` 改成 \`N>2\` 仍然正确**：错误。代码逻辑有误。标准质数判定中，\`N=2\` 时循环可能一次也不执行，但它仍然是质数。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 输入 10 时最后一行会输出 \`11 25\`**：错误。此说法不正确。把 \`i += 1\` 提前会改变奇偶判断时机，程序逻辑会被破坏。
      - **B 输入 5 时最后一行会输出 \`6 9\`**：错误。此说法不正确。把 \`i += 1\` 提前会改变奇偶判断时机，程序逻辑会被破坏。
      - **C 把 \`i += 1\` 移到 \`if\` 前面仍然正确**：正确。把 \`i += 1\` 提前会改变奇偶判断时机，程序逻辑会被破坏。
      - **D 删去 \`if\` 并改成 \`i += 2\` 也可实现**：错误。代码逻辑有误。把 \`i += 1\` 提前会改变奇偶判断时机，程序逻辑会被破坏。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A i == N*N**：错误。代码逻辑有误。当存在 \`i*i == N\` 时，\`N\` 就是完全平方数。
      - **B i*10 == N**：错误。该代码逻辑与题目要求不符，请逐步推演。
      - **C i+i == N**：错误。代码逻辑有误。当存在 \`i*i == N\` 时，\`N\` 就是完全平方数。
      - **D i*i == N**：正确答案。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 8**：错误。可能将结果翻倍计算了，如多循环了一轮或重复累加。正确结果为 4。
      - **B 7**：错误。计算有误。\`i\` 依次为 \`0,2,4,6,8\`，其中满足 \`i<=6\` 的共有 4 个。
      - **C 4**：正确。\`i\` 依次为 \`0,2,4,6,8\`，其中满足 \`i<=6\` 的共有 4 个。
      - **D 1**：错误。计算有误。\`i\` 依次为 \`0,2,4,6,8\`，其中满足 \`i<=6\` 的共有 4 个。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 1**：错误。计算有误。通常为 Scratch、Python、C++ 三种。
      - **B 2**：错误。可能差一错误（off-by-one），如循环条件用了 <= 而非 <。正确结果为 3。
      - **C 3**：正确。通常为 Scratch、Python、C++ 三种。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 感应线圈**：错误。此说法不正确。第一代计算机主要使用电子管。
      - **B 电子管**：正确。第一代计算机主要使用电子管。
      - **C 晶体管**：错误。晶体管是第二代计算机使用的电子元件，此选项不是正确答案。
      - **D 集成电路**：错误。集成电路是第三代计算机使用的电子元件，此选项不是正确答案。
