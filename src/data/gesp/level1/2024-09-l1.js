  ...
  questions: [
    {
      ...
      **解析：**
      ...
      - **B 磁盘**：错误。此说法不正确。磁心存储元件承担的是临时存储数据的作用，对应现代计算机中的内存。
      - **C CPU**：错误。CPU（中央处理器）负责执行指令和逻辑判断，是计算机的"大脑"，此选项不是正确答案。
      - **D 显示器**：错误。此说法不正确。磁心存储元件承担的是临时存储数据的作用，对应现代计算机中的内存。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 阅读源代码**：错误。此说法不正确。跟踪汇编代码属于更底层的调试方式，一般不是一级阶段的常用手段。
      - **B 单步调试**：错误。调试是查找和修复程序错误的过程，不是生成可执行代码，此选项不是正确答案。
      - **C 输出执行中间结果**：错误。此说法不正确。跟踪汇编代码属于更底层的调试方式，一般不是一级阶段的常用手段。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A cout << **：错误。代码逻辑有误。第 4 项的字符串定界符不匹配，会导致编译错误。
      - **B cout << **：错误。代码逻辑有误。第 4 项的字符串定界符不匹配，会导致编译错误。
      - **C cout << **：错误。代码逻辑有误。第 4 项的字符串定界符不匹配，会导致编译错误。
      - **D cout << **：正确。第 4 项的字符串定界符不匹配，会导致编译错误。
      - **B cout << """Hello,GESP!""" << endl;**：错误。代码逻辑有误。第 4 项的字符串定界符不匹配，会导致编译错误。
      - **C cout << "Hello,GESP!\**：错误。代码逻辑有误。第 4 项的字符串定界符不匹配，会导致编译错误。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 14**：错误。计算有误。先算乘法 \`3 * 2 = 6\`，再算减法，结果是 \`4\`。
      - **B 4**：正确。先算乘法 \`3 * 2 = 6\`，再算减法，结果是 \`4\`。
      - **C 1**：错误。计算有误。先算乘法 \`3 * 2 = 6\`，再算减法，结果是 \`4\`。
      - **D 0**：错误。计算有误。先算乘法 \`3 * 2 = 6\`，再算减法，结果是 \`4\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 6**：错误。计算有误。\`10 / 3 = 3\`，\`10 % 3 = 1\`，相加得到 \`4\`。
      - **B 4.3**：错误。计算有误。\`10 / 3 = 3\`，\`10 % 3 = 1\`，相加得到 \`4\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A "6%2={6%2}"**：错误。代码逻辑有误。\`%%\` 会输出 \`%\`，\`6 % 2\` 的结果为 \`0\`。
      - **B 6%2={6%2}**：错误。代码逻辑有误。\`%%\` 会输出 \`%\`，\`6 % 2\` 的结果为 \`0\`。
      - **C 0=0**：错误。此说法不正确。\`%%\` 会输出 \`%\`，\`6 % 2\` 的结果为 \`0\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 将输出整数 7**：错误。此说法不正确。题目没有说明 \`a\` 和 \`b\` 的类型，不同类型下 \`a + b\` 的含义不同。
      - **B 将输出 52，5 和 2 之间没有空格**：错误。变量名不能包含空格，此选项不是正确答案。
      - **C 将输出 5 和 2，5 和 2 之间有空格**：错误。变量名不能包含空格，此选项不是正确答案。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 55**：错误。计算有误。\`0 + 1 + 2 + ... + 9 = 45\`。
      - **B 45**：正确。\`0 + 1 + 2 + ... + 9 = 45\`。
      - **C 10**：错误。计算有误。\`0 + 1 + 2 + ... + 9 = 45\`。
      - **D 9**：错误。计算有误。\`0 + 1 + 2 + ... + 9 = 45\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 55**：错误。计算有误。循环执行 10 次，每次 \`N\` 增加 \`1\`，所以结果是 \`10\`。
      - **B 45**：错误。计算有误。循环执行 10 次，每次 \`N\` 增加 \`1\`，所以结果是 \`10\`。
      - **C 10**：正确。循环执行 10 次，每次 \`N\` 增加 \`1\`，所以结果是 \`10\`。
      - **D 9**：错误。可能差一错误（off-by-one），如循环条件用了 <= 而非 <。正确结果为 10。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 5**：错误。计算有误。\`i\` 依次为 \`1、3、5、7、9\`，每次都满足 \`i % 2 == 1\`，因此 \`N\` 不会增加。
      - **B 4**：错误。计算有误。\`i\` 依次为 \`1、3、5、7、9\`，每次都满足 \`i % 2 == 1\`，因此 \`N\` 不会增加。
      - **C 2**：错误。计算有误。\`i\` 依次为 \`1、3、5、7、9\`，每次都满足 \`i % 2 == 1\`，因此 \`N\` 不会增加。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **B 第5行代码1, 0**：错误。此说法不正确。\`scanf("%d")\` 读到 \`+\` 时停止，所以 \`P = 14\`。此时 \`P % 3 = 2\`，…
      - **C 第7行代码 1**：错误。此说法不正确。\`scanf("%d")\` 读到 \`+\` 时停止，所以 \`P = 14\`。此时 \`P % 3 = 2\`，…
      - **D 第7行代码 0**：错误。此说法不正确。\`scanf("%d")\` 读到 \`+\` 时停止，所以 \`P = 14\`。此时 \`P % 3 = 2\`，…
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 190 20**：错误。此说法不正确。每次循环中 \`i\` 实际增加两次，因此取值为 \`0、2、4、...、18\`，和为 \`90\`，循环次数为 \`…
      - **B 95 10**：错误。此说法不正确。每次循环中 \`i\` 实际增加两次，因此取值为 \`0、2、4、...、18\`，和为 \`90\`，循环次数为 \`…
      - **C 90 19**：错误。此说法不正确。每次循环中 \`i\` 实际增加两次，因此取值为 \`0、2、4、...、18\`，和为 \`90\`，循环次数为 \`…
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 变量 N0 占用额外空间，可以去掉**：错误。此说法不正确。循环结束后 \`N\` 已经变成 \`0\`，应在输出时使用 \`N0\`。
      - **B 代码对所有整数都能计算出正确位数**：错误。此说法不正确。循环结束后 \`N\` 已经变成 \`0\`，应在输出时使用 \`N0\`。
      - **C L11 标记的代码行简单修改后可以对正整数给出正确输出**：正确。循环结束后 \`N\` 已经变成 \`0\`，应在输出时使用 \`N0\`。
      - **D L11 标记的代码行的输出格式有误**：错误。此说法不正确。循环结束后 \`N\` 已经变成 \`0\`，应在输出时使用 \`N0\`。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 如果输入 \`23221 23453 12345 11111 0\`，则最后一行中 ...**：错误。此说法不正确。\`M % 100 / 10\` 取的是十位数，不是百位数。
      - **B 如果输入 \`2322 2345 1234 1111 0\`，程序也能运行**：错误。此说法不正确。\`M % 100 / 10\` 取的是十位数，不是百位数。
      - **C 将 L6 改为 \`M = (M % 1000 / 100);\` 同样能实现题目要...**：错误。应使用除法 / 而非取模 %，两者运算结果不同。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 1**：错误。i) 可以直接判断；ii) 当枚举到 \`i = X + 1\` 时也能成立。iii) 和 iv) 受整数除法影响，不够严谨。
      - **B 2**：正确。i) 可以直接判断；ii) 当枚举到 \`i = X + 1\` 时也能成立。iii) 和 iv) 受整数除法影响，不够严谨。
      - **C 3**：错误。i) 可以直接判断；ii) 当枚举到 \`i = X + 1\` 时也能成立。iii) 和 iv) 受整数除法影响，不够严谨。
      - **D 4**：错误。i) 可以直接判断；ii) 当枚举到 \`i = X + 1\` 时也能成立。iii) 和 iv) 受整数除法影响，不够严谨。
