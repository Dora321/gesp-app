  ...
  questions: [
    // 1-15 单选题
    {
      ...
      **解析：**
      ...
      - **A 量子力学**：错误。2024年诺贝尔物理学奖授予约翰·霍普菲尔德和杰弗里·辛顿，表彰他们在使用人工神经网络进行机器学习方面的基础性发现和发明…
      - **B 黑洞研究**：错误。2024年诺贝尔物理学奖授予约翰·霍普菲尔德和杰弗里·辛顿，表彰他们在使用人工神经网络进行机器学习方面的基础性发现和发明…
      - **C 人工智能/神经网络**：正确。2024年诺贝尔物理学奖授予约翰·霍普菲尔德和杰弗里·辛顿，表彰他们在使用人工神经网络进行机器学习方面的基础性发现和发明。
      - **D 引力波探测**：错误。2024年诺贝尔物理学奖授予约翰·霍普菲尔德和杰弗里·辛顿，表彰他们在使用人工神经网络进行机器学习方面的基础性发现和发明…
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 键盘**：错误。触摸屏可以点击输入，也可以显示输出。
      - **B 打印机**：错误。触摸屏可以点击输入，也可以显示输出。
      - **C 触摸屏**：正确。触摸屏可以点击输入，也可以显示输出。
      - **D 扫描仪**：错误。触摸屏可以点击输入，也可以显示输出。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 123_xiaoyang**：错误。变量名不能以数字开头，不能包含特殊符号@，不能是关键字 int。下划线开头是合法的。
      - **B xiaoyang@bit**：错误。变量名不能以数字开头，不能包含特殊符号@，不能是关键字 int。下划线开头是合法的。
      - **C int**：错误。变量名不能以数字开头，不能包含特殊符号@，不能是关键字 int。下划线开头是合法的。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A include**：错误。public 是类成员访问修饰关键字。include 是预处理指令，printf 是函数，main 是特殊函数名。
      - **B public**：正确。public 是类成员访问修饰关键字。include 是预处理指令，printf 是函数，main 是特殊函数名。
      - **C printf**：错误。public 是类成员访问修饰关键字。include 是预处理指令，printf 是函数，main 是特殊函数名。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **C 字符型和整型**：错误。int 表示整型，double 表示双精度浮点型。
      - **D 整型和字符型**：错误。int 表示整型，double 表示双精度浮点型。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 非门**：错误。与门（AND gate）的逻辑关系是“全 1 则 1”。
      - **B 或门**：错误。与门（AND gate）的逻辑关系是“全 1 则 1”。
      - **C 异或门**：错误。与门（AND gate）的逻辑关系是“全 1 则 1”。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **C 9**：错误。i 从 0 循环到 10，包含 0 和 10，共 11 次。
      - **D 无尽次**：错误。i 从 0 循环到 10，包含 0 和 10，共 11 次。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 1 到 5**：错误。C++ 数组下标从 0 开始。
      - **B 0 到 5**：错误。C++ 数组下标从 0 开始。
      - **C 0 到 4**：正确。C++ 数组下标从 0 开始。
      - **D 1 到 4**：错误。C++ 数组下标从 0 开始。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 48**：错误。'a' 的 ASCII 码是 97，'A' 是 65。
      - **B 65**：错误。'a' 的 ASCII 码是 97，'A' 是 65。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A true**：错误。!true 为 false，false || false 为 false。
      - **B 1**：错误。!true 为 false，false || false 为 false。
      - **C false**：正确。!true 为 false，false || false 为 false。
      - **D 0**：错误。!true 为 false，false || false 为 false。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A #include <iostream.h>**：错误。C++ 标准头文件写法。
      - **B #include iostream**：错误。C++ 标准头文件写法。
      - **C import iostream**：错误。C++ 标准头文件写法。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 9**：错误。1*8+0*4+1*2+1*1 = 11。
      - **B 11**：正确。1*8+0*4+1*2+1*1 = 11。
      - **C 13**：错误。1*8+0*4+1*2+1*1 = 11。
      - **D 15**：错误。1*8+0*4+1*2+1*1 = 11。
