  ...
  questions: [
    // 1-15 单选题
    {
      ...
      **解析：**
      ...
      - **A CCF_GESP**：错误。此说法不正确。变量名不能包含减号。
      - **B CCF-GESP**：正确。变量名不能包含减号。
      - **C CCFGESP**：错误。此说法不正确。变量名不能包含减号。
      - **D ccfGesp**：错误。此说法不正确。变量名不能包含减号。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **C 7**：错误。计算有误。10-3 * 3 % 10 = 10-9 = 1。
      - **D 10**：错误。计算有误。10-3 * 3 % 10 = 10-9 = 1。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A (10+N) % 12**：错误。代码逻辑有误。使用取余运算计算 24 小时制时间。
      - **B (10+N) % 24**：正确。使用取余运算计算 24 小时制时间。
      - **C (10+N) / 24**：错误。整数除法会截断小数部分，请仔细验算。
      - **D 10+N**：错误。此说法不正确。使用取余运算计算 24 小时制时间。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 3**：错误。计算有误。sqrt 返回浮点数。
      - **B 3.0**：正确。sqrt 返回浮点数。
      - **C 9**：错误。计算有误。sqrt 返回浮点数。
      - **D 81**：错误。计算有误。sqrt 返回浮点数。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **B float a = 10;**：错误。此说法不正确。标准定义与初始化。
      - **C char a = 10;**：错误。代码逻辑与正确答案不符。
      - **D a = 10;**：错误。此说法不正确。标准定义与初始化。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 45**：错误。计算有误。1 到 10 之和为 55。
      - **B 50**：错误。计算有误。1 到 10 之和为 55。
      - **C 55**：正确。1 到 10 之和为 55。
      - **D 60**：错误。计算有误。1 到 10 之和为 55。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A +**：错误。此说法不正确。逻辑非 ! 的优先级最高。
      - **B **：错误。此说法不正确。逻辑非 ! 的优先级最高。
      - **C &&**：错误。代码逻辑有误。逻辑非 ! 的优先级最高。
      - **D !**：正确。逻辑非 ! 的优先级最高。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A cin**：错误。代码逻辑有误。cout 是标准输出流。
      - **B scanf**：错误。代码逻辑有误。cout 是标准输出流。
      - **C cout**：正确。cout 是标准输出流。
      - **D abs**：错误。此说法不正确。cout 是标准输出流。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A OK**：错误。此说法不正确。5 / 2 = 2。2 > 2 不成立，输出 KO。
      - **B KO**：正确。5 / 2 = 2。2 > 2 不成立，输出 KO。
      - **C 2.5**：错误。计算有误。5 / 2 = 2。2 > 2 不成立，输出 KO。
      - **D 报错**：错误。此说法不正确。5 / 2 = 2。2 > 2 不成立，输出 KO。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 11**：错误。计算有误。1*8+1*4+0*2+1*1 = 13。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **B // 用于单行注释**：错误。代码逻辑有误。注释可以写在程序的任何位置。
    ...
    },
    {
      ...
      **解析：**
      ...
      - **A 图灵机**：错误。此说法不正确。现代通用计算机大多采用冯·诺依曼架构。
      - **B 冯·诺依曼架构**：正确。现代通用计算机大多采用冯·诺依曼架构。
      - **C 哈佛架构**：错误。此说法不正确。现代通用计算机大多采用冯·诺依曼架构。
      - **D 神经元架构**：错误。此说法不正确。现代通用计算机大多采用冯·诺依曼架构。
