    ...
    questions: [
        {
            ...
            **解析：**
            \`p\` 是指向 \`a\` 的指针，\`*p\` 访问的就是 \`a\` 的内存。\`*p = *p+1\` 等同于 \`a = a+1\`，所以 \`a\` 变为 43。
            
            - **A 42**：错误。可能差一错误（off-by-one），如循环条件用了 <= 而非 <。正确结果为 43。
            - **B 43**：正确。\`p\` 是指向 \`a\` 的指针，\`*p\` 访问的就是 \`a\` 的内存。\`*p = *p+1\` 等同于 \`a = a+1\`，所以 \`a\…
            - **C 编译错误**：错误。编译是将高级语言源代码翻译成机器可执行代码的过程，此选项不是正确答案。
            - **D 不确定**：错误。\`p\` 是指向 \`a\` 的指针，\`*p\` 访问的就是 \`a\` 的内存。\`*p = *p+1\` 等同于…
        ...
        },
        {
            ...
            **解析：**
            数组名是常量指针（Pointer Constant），其指向的地址不可更改。因此不能进行自增（\`arr++\`）操作。
        ...
        },
        {
            ...
            **解析：**
            \`arr+1\` 指向第二行首地址，\`*(arr+1)\` 得到第二行数组名（首元素地址），\`*(arr+1)+2\` 指向第二行第三列元素的地址，解引用得到 \`arr[1][2]\` 的值 6。
            
            - **A 2**：错误。\`arr+1\` 指向第二行首地址，\`*(arr+1)\` 得到第二行数组名（首元素地址），\`*(arr+1)+2…
            - **B 5**：错误。可能差一错误（off-by-one），如循环条件用了 <= 而非 <。正确结果为 6。
            - **C 4**：错误。\`arr+1\` 指向第二行首地址，\`*(arr+1)\` 得到第二行数组名（首元素地址），\`*(arr+1)+2…
            - **D 6**：正确。\`arr+1\` 指向第二行首地址，\`*(arr+1)\` 得到第二行数组名（首元素地址），\`*(arr+1)+2\` 指向第二行第三列元素的地址，解引用…
        ...
        },
        {
            ...
            **解析：**
            \`add(2)\` 调用时 \`b\` 使用默认参数 1，结果为 3；\`add(2, 3)\` 调用时 \`b\` 被显式赋值 3，结果为 5。输出为 3 5。
            
            - **A 3 5**：正确。\`add(2)\` 调用时 \`b\` 使用默认参数 1，结果为 3；\`add(2, 3)\` 调用时 \`b\` 被显式赋值 3，结果为 5。输出为 3 …
            - **B 编译失败：定义处少了默认参数**：错误。对函数的定义或调用规则理解有误。
            - **C 运行错误**：错误。\`add(2)\` 调用时 \`b\` 使用默认参数 1，结果为 3；\`add(2, 3)\` 调用时 \`b\` …
            - **D 链接失败：未定义引用**：错误。\`add(2)\` 调用时 \`b\` 使用默认参数 1，结果为 3；\`add(2, 3)\` 调用时 \`b\` …
        ...
        },
        {
            ...
            **解析：**
            \`foo()\` 中定义了局部变量 \`x\`，屏蔽了全局变量，输出 10；\`bar()\` 中没有局部变量 \`x\`，访问全局变量，输出 5。
            
            - **A 5 5**：错误。\`foo()\` 中定义了局部变量 \`x\`，屏蔽了全局变量，输出 10；\`bar()\` 中没有局部变量 \`x…
            - **B 10 10**：错误。\`foo()\` 中定义了局部变量 \`x\`，屏蔽了全局变量，输出 10；\`bar()\` 中没有局部变量 \`x…
            - **C 5 10**：错误。\`foo()\` 中定义了局部变量 \`x\`，屏蔽了全局变量，输出 10；\`bar()\` 中没有局部变量 \`x…
            - **D 10 5**：正确。\`foo()\` 中定义了局部变量 \`x\`，屏蔽了全局变量，输出 10；\`bar()\` 中没有局部变量 \`x\`，访问全局变量，输出 5。
        ...
        },
        {
            ...
            **解析：**
            \`increaseA\` 是值传递，不修改原变量，输出 5；\`increaseB\` 是指针传递，修改原变量内存，输出 6。
            
            - **A 6 7**：错误。\`increaseA\` 是值传递，不修改原变量，输出 5；\`increaseB\` 是指针传递，修改原变量内存，输…
            - **B 6 6**：错误。\`increaseA\` 是值传递，不修改原变量，输出 5；\`increaseB\` 是指针传递，修改原变量内存，输…
            - **C 5 6**：正确。\`increaseA\` 是值传递，不修改原变量，输出 5；\`increaseB\` 是指针传递，修改原变量内存，输出 6。
            - **D 5 5**：错误。\`increaseA\` 是值传递，不修改原变量，输出 5；\`increaseB\` 是指针传递，修改原变量内存，输…
        ...
        },
        {
            ...
            **解析：**
            C++ 中结构体可以使用列表初始化（花括号形式）。
            
            - **A Point p = (1, 2);**：错误。C++ 中结构体可以使用列表初始化（花括号形式）。
            - **B Point p = {1, 2};**：正确。C++ 中结构体可以使用列表初始化（花括号形式）。
            - **C Point p = new {1, 2};**：错误。C++ 中结构体可以使用列表初始化（花括号形式）。
            - **D Point p = <1, 2>;**：错误。C++ 中结构体可以使用列表初始化（花括号形式）。
        ...
        },
        {
            ...
            **解析：**
            \`birthday\` 函数接受结构体的引用 \`Cat& c\`，因此在函数内部对 \`age\` 的修改会反映到原始对象 \`kitty\` 上。
            
            - **A Mimi 2**：错误。\`birthday\` 函数接受结构体的引用 \`Cat& c\`，因此在函数内部对 \`age\` 的修改会反映到原…
            - **B Mimi 3**：正确。\`birthday\` 函数接受结构体的引用 \`Cat& c\`，因此在函数内部对 \`age\` 的修改会反映到原始对象 \`kitty\` 上。
            - **C kitty 3**：错误。\`birthday\` 函数接受结构体的引用 \`Cat& c\`，因此在函数内部对 \`age\` 的修改会反映到原…
            - **D kitty 2**：错误。\`birthday\` 函数接受结构体的引用 \`Cat& c\`，因此在函数内部对 \`age\` 的修改会反映到原…
        ...
        },
        {
            ...
            **解析：**
            选择排序在交换最小元素时可能会破坏相同元素的相对位置，因此是不稳定的。冒泡排序和插入排序是稳定的。
        ...
        },
        {
            ...
            **解析：**
            选择排序寻找未排序区间的最小值索引，若发现更小的则更新 \`minIndex\`，内循环结束后将最小值与当前位置 \`i\` 交换。
        ...
        },
        {
            ...
            **解析：**
            插入排序将当前 \`key\` 与前面已排序序列比较，若 \`arr[j] > key\` 则后移元素，最后在空出的位置 \`j+1\` 放置 \`key\`。
        ...
        },
        {
            ...
            **解析：**
            当输入数组已经有序时，插入排序只需遍历一次，复杂度为 $O(N)$；当数组逆序时，每次需移动所有元素，复杂度为 $O(N^2)$。
        ...
        },
        {
            ...
            **解析：**
            状态转移方程为 \`f(n) = f($n-1$)+f(n-2)\`。计算出 \`current\` 后，更新 \`prev2\` 为旧的 \`prev1\`，\`prev1\` 为新的 \`current\` 供下轮迭代。
        ...
        },
        {
            ...
            **解析：**
            代码包含三层嵌套循环，每层循环次数均与 $n$ 成线性关系，因此总复杂度为 $O(n^3)$。
            
            - **A $O(N)$**：错误。代码包含三层嵌套循环，每层循环次数均与 $n$ 成线性关系，因此总复杂度为 $O(n^3)$。
            - **B $O(n log n)$**：错误。代码包含三层嵌套循环，每层循环次数均与 $n$ 成线性关系，因此总复杂度为 $O(n^3)$。
            - **C $O(n^3)$**：正确。代码包含三层嵌套循环，每层循环次数均与 $n$ 成线性关系，因此总复杂度为 $O(n^3)$。
            - **D $O(3^n)$**：错误。代码包含三层嵌套循环，每层循环次数均与 $n$ 成线性关系，因此总复杂度为 $O(n^3)$。
        ...
        },
        {
            ...
            **解析：**
            未被捕获（uncaught）的异常会导致程序调用 \`std::terminate\` 终止，但并不是语法上要求“必须”捕获（例如可以留给调用者处理）。D 选项表述过于绝对（应当说“推荐捕获”或“不捕获会导致异常终止”）。
        ...
        },
        {
            ...
            **解析：**
            在函数 \`f\` 内部，定义 \`int x = x+1\` 时，等号右边的 \`x\` 是正在定义的局部变量本身。此时局部变量尚未初始化完成，其值是不确定的（UB），且它屏蔽了全局变量。
        ...
        },
        {
            ...
            **解析：**
            通过指针传递地址，在函数内修改指针指向的内容会改变原变量的值。
        ...
        },
        {
            ...
            **解析：**
            二维数组传参时，第二维的大小必须指定，第一维可以省略。\`int arr[][4]\` 是正确且常见的写法。
        ...
        },
        {
            ...
            **解析：**
            插入排序常数项较小，且在近乎有序的情况下效率极高，适合处理小型序列。
        ...
        },
        {
            ...
            **解析：**
            冒泡排序第一轮会将最大值 5 交换到最后。过程：(4,1)->1,4; (4,3)->3,4; (4,1)->1,4; (4,5)->4,5; (5,2)->2,5。最终结果：\`{1, 3, 1, 4, 2, 5}\`。
        ...
        },
        {
            ...
            **解析：**
            选择排序的逻辑是：每次扫描全数组找最小值。无论数据是否有序，它都会进行相同次数的比较，因此复杂度恒定为 $O(N^2)$。
...
`,
      ...
      explanation: `
      **解析：**
      该问题可以转化为求“全 1 子矩阵的最大面积”。由于数据范围较小 (n,m <= 500)，可以使用单调栈优化：对每一行，维护以该行为底向上连续 1 的高度。这样每一行就变成了一个“直方图最大矩形”问题，利用单调栈可以在 O(m) 内求解，总复杂度 O(n*m)。
      `,
...
`,
      ...
      explanation: `
      **解析：**
      重排后能构成的最长“连续段”，本质上是原数组去重排序后，能够构成的最长“值连续整数序列”。解法：先排除重复元素（因为连续段内元素互不相同），排序后遍历，记录最长的相邻差值为 1 的区间。
      `,
      template: "#include <bits/stdc++.h>\nusing namespace std;\n\nint main() {\n    ios::sync_with_stdio(false);\n    cin.tie(nullptr);\n\n    // 在此编写代码\n\n    return 0;\n}",
