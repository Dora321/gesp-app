    ...
    questions: [
        {
            ...
            **解析：**
            ...
            - **D 不确定**：错误。此说法不正确。\`p\` 是指向 \`a\` 的指针，\`*p\` 访问的就是 \`a\` 的内存。\`*p = *p+1\` 等同于…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 2**：错误。计算有误。\`arr+1\` 指向第二行首地址，\`*(arr+1)\` 得到第二行数组名（首元素地址），\`*(arr+1)+2…
            - **B 5**：错误。可能差一错误（off-by-one），如循环条件用了 <= 而非 <。正确结果为 6。
            - **C 4**：错误。计算有误。\`arr+1\` 指向第二行首地址，\`*(arr+1)\` 得到第二行数组名（首元素地址），\`*(arr+1)+2…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **C 运行错误**：错误。此说法不正确。\`add(2)\` 调用时 \`b\` 使用默认参数 1，结果为 3；\`add(2, 3)\` 调用时 \`b\` …
            - **D 链接失败：未定义引用**：错误。此说法不正确。\`add(2)\` 调用时 \`b\` 使用默认参数 1，结果为 3；\`add(2, 3)\` 调用时 \`b\` …
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 5 5**：错误。此说法不正确。\`foo()\` 中定义了局部变量 \`x\`，屏蔽了全局变量，输出 10；\`bar()\` 中没有局部变量 \`x…
            - **B 10 10**：错误。此说法不正确。\`foo()\` 中定义了局部变量 \`x\`，屏蔽了全局变量，输出 10；\`bar()\` 中没有局部变量 \`x…
            - **C 5 10**：错误。此说法不正确。\`foo()\` 中定义了局部变量 \`x\`，屏蔽了全局变量，输出 10；\`bar()\` 中没有局部变量 \`x…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 6 7**：错误。此说法不正确。\`increaseA\` 是值传递，不修改原变量，输出 5；\`increaseB\` 是指针传递，修改原变量内存，输…
            - **B 6 6**：错误。此说法不正确。\`increaseA\` 是值传递，不修改原变量，输出 5；\`increaseB\` 是指针传递，修改原变量内存，输…
            - **C 5 6**：正确。\`increaseA\` 是值传递，不修改原变量，输出 5；\`increaseB\` 是指针传递，修改原变量内存，输出 6。
            - **D 5 5**：错误。此说法不正确。\`increaseA\` 是值传递，不修改原变量，输出 5；\`increaseB\` 是指针传递，修改原变量内存，输…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A Point p = (1, 2);**：错误。代码逻辑有误。C++ 中结构体可以使用列表初始化（花括号形式）。
            - **B Point p = {1, 2};**：正确。C++ 中结构体可以使用列表初始化（花括号形式）。
            - **C Point p = new {1, 2};**：错误。代码逻辑有误。C++ 中结构体可以使用列表初始化（花括号形式）。
            - **D Point p = <1, 2>;**：错误。代码逻辑有误。C++ 中结构体可以使用列表初始化（花括号形式）。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A Mimi 2**：错误。此说法不正确。\`birthday\` 函数接受结构体的引用 \`Cat& c\`，因此在函数内部对 \`age\` 的修改会反映到原…
            - **B Mimi 3**：正确。\`birthday\` 函数接受结构体的引用 \`Cat& c\`，因此在函数内部对 \`age\` 的修改会反映到原始对象 \`kitty\` 上。
            - **C kitty 3**：错误。此说法不正确。\`birthday\` 函数接受结构体的引用 \`Cat& c\`，因此在函数内部对 \`age\` 的修改会反映到原…
            - **D kitty 2**：错误。此说法不正确。\`birthday\` 函数接受结构体的引用 \`Cat& c\`，因此在函数内部对 \`age\` 的修改会反映到原…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A $O(N)$**：错误。此说法不正确。代码包含三层嵌套循环，每层循环次数均与 $n$ 成线性关系，因此总复杂度为 $O(n^3)$。
            - **B $O(n log n)$**：错误。此说法不正确。代码包含三层嵌套循环，每层循环次数均与 $n$ 成线性关系，因此总复杂度为 $O(n^3)$。
            - **C $O(n^3)$**：正确。代码包含三层嵌套循环，每层循环次数均与 $n$ 成线性关系，因此总复杂度为 $O(n^3)$。
            - **D $O(3^n)$**：错误。此说法不正确。代码包含三层嵌套循环，每层循环次数均与 $n$ 成线性关系，因此总复杂度为 $O(n^3)$。
