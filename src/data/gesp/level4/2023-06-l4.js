...
**选项逐项分析：**
- **A 指针变量中存储的是内存地址**：✅ 正确。
- **B 定义指针变量时必须指定其指向的类型**：✅ 正确。这决定了指针运算的步长。
- **C 只能指向基本类型变量**：❌ 错误。指针可以指向任何类型，包括自定义结构体、数组，甚至是另一个指针变量（即多级指针）。
- **D 指针变量指向的内存地址不一定能够合法访问**：✅ 正确（如野指针或越界地址）。
...
**选项逐项分析：**
- **A 一维数组在内存中一定是连续存放的**：✅ 正确。
- **B 二维数组是一维数组的一维数组**：✅ 正确。在逻辑上，二维数组的每一行都是一个一维数组。
- **C 二维数组中的每个一维数组在内存中都是连续存放的**：✅ 正确。
- **D 二维数组在内存中可以不是连续存放的**：❌ 错误。在 C++ 中，静态定义的数组（无论维数）在内存中都是按行优先原则**连续**排列的。
...
- **B 60 / C 120**：❌ 错误。
- **D 240**：✅ 正确。
  1. \`array[3][10]\` 共有 $3 \times 10 = 30$ 个元素。
  2. 元素类型为 \`double\`，在 C++ 中通常占用 8 字节。
  3. 总内存占用 = $30 \times 8 = 240$ 字节。

**考点：** 数组内存占用计算（行 $\times$ 列 $\times$ sizeof(类型)）。`,
...
- **A 2字节 / B 4字节**：❌ 错误。
- **C 8字节**：✅ 正确。
  1. 二维数组 \`int array[5][3]\` 每行有 3 个元素。
  2. \`array[1][2]\` 的线性序号（从 0 开始）：$1 \times 3 + 2 = 5$。
  3. \`array[2][1]\` 的线性序号：$2 \times 3 + 1 = 7$。
  4. 序号差值为 $7 - 5 = 2$ 个元素。
  5. 每个 \`int\` 占用 4 字节，故距离为 $2 \times 4 = 8$ 字节。
- **D 无法确定**：❌ 错误。

**考点：** 二维数组的行优先排列规则与内存偏移计算。`,
            tags: ["数组内存", "单选题", "GESP4级"]
        },
        {
            id: 10,
...
- **A {1, 3, 3, 4, 5}**：✅ 正确。
  1. \`a = {1, 2, 3, 4, 5}\`。
  2. \`int *p = &a[2];\`：指针 \`p\` 指向 \`a[2]\`（值为 3）。
  3. \`a[1] = *p;\`：将 \`p\` 指向的值（即 3）赋给 \`a[1]\`。
  4. \`a[1]\` 的值从 2 变为 3。
- **B / C / D**：❌ 错误。
...
**选项逐项分析：**
- **A 3 / B 30 / C 33**：❌ 错误（注：考题中通常选项 C 是正确答案）。
- **D 无法确定**：❌ 错误。
*(注：根据代码逻辑分析)*
1. \`array[i][j] = i*10+j\` 初始化数组：
   - \`array[0][0] = 0, array[0][1] = 1, array[0][2] = 2\`
   - \`array[1][0] = 10, array[1][1] = 11, array[1][2] = 12\`
   - \`array[2][0] = 20, array[2][1] = 21, array[2][2] = 22\`
2. 累加主对角线元素 \`array[i][i]\`：
   - \`sum = array[0][0] + array[1][1] + array[2][2] = 0 + 11 + 22 = 33\`。
3. 最终输出 \`33\`。
...
**选项逐项分析：**
- **A array[min] > array[j]**：✅ 正确。选择排序的核心是在每一轮中找到剩余部分的最小值。如果当前遍历到的元素 \`array[j]\` 小于目前记录的最小值 \`array[min]\`，则更新最小值下标 \`min = j\`。
- **B / C / D**：❌ 错误。

**考点：** 选择排序（Selection Sort）的算法逻辑。`,
