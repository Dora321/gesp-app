...
**考点：** 二分查找算法（Binary Search）的模拟计算。`,
        ...
        },
        {
            ...
            answer: 1,
            score: 2,
            explanation: `**答案：C (分治算法总是比其他算法更高效)**

**选项逐项分析：**
- **A 分治算法通常配合递归实现**：✅ 正确。分治法的“分”与“合”天然适合递归结构。
- **B 归并排序是典型的分治算法**：✅ 正确。它通过递归地将数组分成两半，分别排序后再合并。
- **C 分治算法总是比其他算法更高效**：❌ 错误。分治算法的效率取决于子问题的规模和合并过程的复杂度。在某些情况下（如数据量极小），其开销可能高于简单的暴力法或迭代法。
- **D 快速排序采用了分治策略**：✅ 正确。它通过“基准值”将数组划分为两部分进行递归处理。

**考点：** 分治算法（Divide and Conquer）的概念、特点与局限性。`,
            tags: ["客观题", "单选题", "GESP4级"]
        },
        {
            id: 12,
...
- **B 5**：✅ 正确。
  1. 初始序列：\`{5, 2, 8, 1, 9}\`。
  2. 排序后（升序）：\`{1, 2, 5, 8, 9}\`。
  3. \`a[2]\` 访问的是排序后索引为 2 的元素（即第 3 个元素）。
  4. 结果为 5。

**考点：** \`std::sort\` 函数的使用与数组索引访问。`,
            tags: ["客观题", "单选题", "GESP4级"]
        },
        {
            id: 13,
