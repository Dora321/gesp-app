...
const programmingQuestions = [
    {
        ...
        description: "现有一片山地，可以视为一个 $N$ 行 $M$ 列的网格图，第 $i$ 行 $j$ 列的海拔为 $h_{i,j}$。如果一个单元格的海拔不高于其所有相邻单元格（相邻包括上、下、左、右、左上、右上、左下、右下，最多 8 个方向）的海拔，则称该单元格为山谷。请你数一数该片山地中有多少山谷。",
        inputDescription: "第一行包含 2 个整数 $N, M$，表示山地的大小。之后 $N$ 行，每行包含 $M$ 个整数 $h_{i,1}, h_{i,2}, \\dots, h_{i,M}$，表示海拔。",
    ...
    },
    {
        ...
        description: "商店推出了许多礼盒，每个礼盒中包含 $k$ 件商品，每件商品都有一个价格。现在需要对这些礼盒进行排序，排序规则如下：\n1. 先按礼盒总价格从小到大排序；\n2. 如果总价格相同，则按礼盒中最贵商品的价格从小到大排序；\n3. 如果仍然相同，则按礼盒中最后进店商品的价格（即输入的最后一个价格）从小到大排序；\n4. 如果上述规则都相同，则按礼盒原本的编号（从 1 开始）从小到大排序。",
        inputDescription: "第一行包含两个整数 $n, k$，表示礼盒数量和每个礼盒中的商品数量。接下来的 $n$ 行，每行包含 $k$ 个整数，表示对应礼盒中商品的价格。",
        outputDescription: "输出一行，包含 $n$ 个整数，用空格分隔，表示排序后的礼盒编号。",
        samples: [
            {
                input: "4 3\n3 5 2\n4 1 5\n2 2 4\n3 4 3",
                output: "3 4 2 1"
