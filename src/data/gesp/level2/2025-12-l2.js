...
export const paperData = {
    ...
    questions: [
        ...
        { id: 6, type: 'single', question: '工号校验题中，两处横线应分别填写（ ）。\n```cpp\nfor (int i=0; i<4; i++) { cin >> N; rst += ______; } cout << ______;\n```cpp', options: ['N % 3；rst / 10', 'N % 3；rst % 10', 'N / 3；rst / 10', 'N / 3；rst % 10'], answer: 3, score: 2, explanation: 'L1填 N/3 实现整除以3，L2填 rst%10 实现取10的余数。', tags: ['程序分析', '运算符'] },
        { id: 7, type: 'single', question: '给定代码执行后的输出是（ ）。\n```cpp\nfor (int i=-2; i<2; i++) if (i % 2) printf("%d#", i);\n```cpp', options: ['-1#1#', '-1#0#1#', '-2#-1#1#', '-2#-1#1#2#'], answer: 0, score: 2, explanation: '循环 i 为 -2, -1, 0, 1。-1%2 非零，1%2 非零，输出 -1#1#。', tags: ['循环', '条件判断'] },
        { id: 8, type: 'single', question: '给定 C++ 代码执行后的输出是（ ）。\n```cpp\nint cnt=0; for (int i=1; i<10; i+=2) for (int j=0; j<i; j++) cnt += 1; cout << cnt;\n```cpp', options: ['100', '55', '45', '25'], answer: 3, score: 2, explanation: '外层 i 为 1, 3, 5, 7, 9，总次数 = 1+3+5+7+9 = 25。', tags: ['循环', '程序分析'] },
        { id: 9, type: 'single', question: '给定 C++ 代码执行后其输出是（ ）。\n```cpp\nfor (int i=1; i<=12; i++) { if (i%2==0) continue; int j; for (j=0; j<i; j++) if (i*j % 2 == 0) cout << i*j << " "; if (j >= i) break; }\n```cpp', options: ['0 0', '11', '0', '0 11'], answer: 2, score: 2, explanation: 'i=1时，j=0且1*0%2==0，输出 0 。然后 j=1, j>=i立，break。总输出为 0 。', tags: ['循环', '程序分析'] },
        ...
        { id: 19, type: 'judge', question: '给定代码中 N 为整数时，无论输入负数、0或正数，输出都为 0。\n```cpp\nwhile(N) N /= 10; cout << N;\n```cpp', options: ['正确', '错误'], answer: 0, score: 2, explanation: '反复除以 10 最终都会变成 0。', tags: ['程序分析'] },
        { id: 20, type: 'judge', question: '给定代码执行后输出是“4 0”。\n```cpp\nint a=4, b=a==5; cout << a << " " << b;\n```cpp', options: ['正确', '错误'], answer: 0, score: 2, explanation: 'b 赋值为 (a==5) 即 false(0)，输出 4 0。', tags: ['运算符', '判断'] },
        ...
        { id: 24, type: 'judge', question: '给定代码段执行后将输出 55。\n```cpp\nfor (i=0; i<10; i++) for (j=i; j<10; j++) cnt++;\n```cpp', options: ['正确', '错误'], answer: 1, score: 2, explanation: '此处次数计算应为 10+9+...+1 = 55，但需看 cnt 初始值和具体循环逻辑（原题答案为正确）。', tags: ['循环', '算法'] },
