...
export const paperData = {
    ...
    questions: [
        ...
        { id: 4, type: 'single', question: '下面 C++ 代码用于获得正整数 N 的第 M 位数，横线处应填入的代码是（ ）。\nfor (int i = 0; i < (M - 1); i++) div *= 10;\ncout << (______________);', options: ['N % div / 10', 'N / div / 10', 'N % div % 10', 'N / div % 10'], answer: 3, score: 2, explanation: '先通过除以 div 将目标位移到个位，再对 10 取模。', tags: ['程序分析'] },
        ...
        { id: 8, type: 'single', question: '下面代码执行后其输出是（ ）。\nfor (i = 1; i < 12; i++){ if (i % 2 == 0) continue; for (j = 0; j < i; j++) if (i * j % 2) break; }\nif(i >= 12) cout << (i * j);', options: ['110', '12', '不确定', '无输出'], answer: 1, score: 2, explanation: '循环结束时 i=12。最后一次内循环 i=11 时，j 增加到满足 break 或循环结束。分析得 j=1。输出 12×1 = 12。', tags: ['程序分析'] },
        { id: 9, type: 'single', question: '阅读代码，说法正确的是（ ）。\nwhile (b != 0){ remainder = a % b; a = b; b = remainder; }', options: ['b 不能为 0', 'a 必须小于 b', 'a 和 b 必须为正整数', '如果 a 输入为 0，则输出值的绝对值都是 abs(b)'], answer: 3, score: 2, explanation: '这是辗转相除法求最大公约数。', tags: ['算法'] },
        ...
        { id: 14, type: 'single', question: '下面 C++ 代码执行，其输出是（ ）。\nint a=9, b=27; a = \'a\' + \'b\'; b = \'a\' - \'b\'; a = a - b; cout << a << \' \' << b;', options: ['196 -1', '27 9', '98 97', '不确定'], answer: 0, score: 2, explanation: '\'a\'=97, \'b\'=98。a=195, b=-1, a=195-(-1)=196。', tags: ['字符运算'] },
