    ...
    questions: [
        {
            ...
            **解析：**
            ...
            - **A 大电脑模型**：错误。大模型在人工智能领域通常指基于海量数据训练的、具有巨大参数量的大语言模型（LLM）。
            - **B 大规模智能**：错误。大模型在人工智能领域通常指基于海量数据训练的、具有巨大参数量的大语言模型（LLM）。
            - **C 智能的单位**：错误。大模型在人工智能领域通常指基于海量数据训练的、具有巨大参数量的大语言模型（LLM）。
            - **D 大语言模型**：正确。大模型在人工智能领域通常指基于海量数据训练的、具有巨大参数量的大语言模型（LLM）。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A (yr%400==0) || (yr%4==0)**：错误。应使用逻辑与 && 而非逻辑或 ||，两者语义不同。
            - **B (yr%400==0) || (yr%4==0 && yr%100!=0)**：正确。答案依据官方答案。
            - **C (yr%400==0) && (yr%4==0)**：错误。应使用逻辑或 || 而非逻辑与 &&，两者语义不同。
            - **D (yr%400==0) && (yr%4==0 && yr%100!=0)**：错误。应使用逻辑或 || 而非逻辑与 &&，两者语义不同。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A five-Star**：正确。答案依据官方答案。
            - **B five_star**：错误。此说法不正确。答案依据官方答案。
            - **C fiveStar**：错误。此说法不正确。答案依据官方答案。
            - **D _fiveStar**：错误。此说法不正确。答案依据官方答案。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A for(int i=0; i<10; i+=1)**：正确。答案依据官方答案。
            - **B for(int i=1; i<=10; i++)**：错误。递增方式不正确，应使用复合赋值运算符。
            - **C for(int i=10; i>0; i--)**：错误。代码逻辑与正确答案不符。
            - **D for(int i=10; i<1; i++)**：错误。递增方式不正确，应使用复合赋值运算符。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 1**：正确。5 % 2 为 1 (真)，5 % 3 为 2 (真)。1 && 2 在逻辑运算中为真 (true)，输出时整数显示为 1。
            - **B 2**：错误。可能差一错误（off-by-one），如循环条件用了 <= 而非 <。正确结果为 1。
            - **C true**：错误。5 % 2 为 1 (真)，5 % 3 为 2 (真)。1 && 2 在逻辑运算中为真 (true)，输出时整数显示为 …
            - **D false**：错误。5 % 2 为 1 (真)，5 % 3 为 2 (真)。1 && 2 在逻辑运算中为真 (true)，输出时整数显示为 …
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A Jan**：错误。输入 1，命中 case 1 输出 "Jan "。由于没有 break，继续执行 case 3 输出 "Mar "，然后…
            - **B Mar**：错误。输入 1，命中 case 1 输出 "Jan "。由于没有 break，继续执行 case 3 输出 "Mar "，然后…
            - **C Jan Mar**：正确。输入 1，命中 case 1 输出 "Jan "。由于没有 break，继续执行 case 3 输出 "Mar "，然后 break。
            - **D 以上均不对**：错误。输入 1，命中 case 1 输出 "Jan "。由于没有 break，继续执行 case 3 输出 "Mar "，然后…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 如果先后输入 1 和 1 ，则将输出 1**：错误。此说法不正确。答案依据官方答案。
            - **B 如果先后输入 0 和 1 或者 1 和 0 ，则将输出 3**：错误。此说法不正确。答案依据官方答案。
            - **C 如果先后输入 0 和 0 ，则将输出 2**：错误。此说法不正确。答案依据官方答案。
            - **D 如果先后输入 0 和 0 ，则将输出 4**：正确。答案依据官方答案。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 第 1 横线处应填入： N / 2 第 2 横线处应填入： N-M5-M2**：错误。此说法不正确。答案依据官方答案。
            - **B 第 1 横线处应填入： (N-M5 * 5) / 2 第 2 横线处应填入： N...**：正确。答案依据官方答案。
            - **C 第 1 横线处应填入： N-M5 * 5 / 2 第 2 横线处应填入： N-M...**：错误。此说法不正确。答案依据官方答案。
            - **D 第 1 横线处应填入： (N-M5 * 5) / 2 第 2 横线处应填入： N...**：错误。此说法不正确。答案依据官方答案。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 1**：错误。a=5(真), b=0(假)。a&&b为假；!(a||b)即!(真)为假；a||b为真。输出 3。
            - **B 2**：错误。可能差一错误（off-by-one），如循环条件用了 <= 而非 <。正确结果为 3。
            - **C 3**：正确。a=5(真), b=0(假)。a&&b为假；!(a||b)即!(真)为假；a||b为真。输出 3。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 10**：错误。追踪循环：i偶数时内层循环不break；i奇数时j=1即break。经计算loopCount为18。
            - **B 45**：错误。追踪循环：i偶数时内层循环不break；i奇数时j=1即break。经计算loopCount为18。
            - **C 18**：正确。追踪循环：i偶数时内层循环不break；i奇数时j=1即break。经计算loopCount为18。
            - **D 25**：错误。追踪循环：i偶数时内层循环不break；i奇数时j=1即break。经计算loopCount为18。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 执行上⾯代码如果输入 0 ，将终⽌循环**：错误。对循环执行过程的分析有误。
            - **B 执行上⾯代码能实现所有⾮ 0 整数的求和**：错误。此说法不正确。while(N) 表示 N 不为 0 时继续循环。输入 0 会终止循环。改为 while(N==0) 反而会导致逻辑错误…
            - **C 执行上⾯代码第一次输入 0 ，最后将输出 0**：错误。此说法不正确。while(N) 表示 N 不为 0 时继续循环。输入 0 会终止循环。改为 while(N==0) 反而会导致逻辑错误…
            - **D 执行上⾯代码将陷入死循环，可将while (N)改为while (N==0)**：正确。while(N) 表示 N 不为 0 时继续循环。输入 0 会终止循环。改为 while(N==0) 反而会导致逻辑错误。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 如果输入正整数，上⾯代码能正确判断 N 是否为质数**：错误。对条件表达式的逻辑关系理解有误。
            - **B 如果输入整数，上⾯代码能正确判断 N 是否为质数**：错误。对条件表达式的逻辑关系理解有误。
            - **C 如果输入大于等于 0 的整数，上⾯代码能正确判断 N 是否质数**：错误。对条件表达式的逻辑关系理解有误。
            - **D 如将Flag = true修改为Flag = N>=2? true:false则...**：正确。质数定义为大于 1 的自然数。原代码对 N=0,1 会判定为质数，加入 N>=2 判断后更严谨。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 当前代码能实现预期效果，⽆需调整代码**：正确。外层循环控制行数，内层循环控制每行星号数，每行结束后输出换行，逻辑正确。
            - **B 如果cout << endl;移到循环 L2 内部，则可实现预期效果**：错误。代码逻辑有误。外层循环控制行数，内层循环控制每行星号数，每行结束后输出换行，逻辑正确。
            - **C 如果cout << endl; 移到循环 L1 外部，则可实现预期效果**：错误。代码逻辑有误。外层循环控制行数，内层循环控制每行星号数，每行结束后输出换行，逻辑正确。
            - **D 删除cout << endl;行，则可实现预期效果**：错误。代码逻辑有误。外层循环控制行数，内层循环控制每行星号数，每行结束后输出换行，逻辑正确。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 5 2**：错误。第一次：a=5, b=2, a>b -> a=3, b=2; 第二次：a=3, b=2, a>b -> a=1, b=2…
            - **B 1 2**：正确。第一次：a=5, b=2, a>b -> a=3, b=2; 第二次：a=3, b=2, a>b -> a=1, b=2. 输出 1 2。
            - **C 1 0**：错误。第一次：a=5, b=2, a>b -> a=3, b=2; 第二次：a=3, b=2, a>b -> a=1, b=2…
            - **D 以上均不对**：错误。第一次：a=5, b=2, a>b -> a=3, b=2; 第二次：a=3, b=2, a>b -> a=1, b=2…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 输出 21 是 7 的超级幸运数**：错误。此说法不正确。21 能被 7 整除但不包含 7，故为幸运数。
            - **B 输出 21 是 7 的幸运数**：正确。21 能被 7 整除但不包含 7，故为幸运数。
            - **C 输出 21 非 7 的幸运数**：错误。此说法不正确。21 能被 7 整除但不包含 7，故为幸运数。
            - **D 以上均不对**：错误。此说法不正确。21 能被 7 整除但不包含 7，故为幸运数。
...
`,
      ...
      explanation: `
      **解析：**
      对每个 a，枚举正整数 x，再检查 a-x^2 是否也是某个正整数的平方。只要找到一组 (x,y) 即可输出 Yes。
      `,
      tags: ["编程题", "枚举", "完全平方数"],
...
`,
      ...
      explanation: `
      **解析：**
      从 1 到 n 枚举每个整数，把它转成十进制字符串或不断取模，统计数字 k 出现的次数。
      `,
      tags: ["编程题", "枚举", "数位统计"],
      template: "#include <bits/stdc++.h>\nusing namespace std;\n\nint main() {\n    ios::sync_with_stdio(false);\n    cin.tie(nullptr);\n\n    // 在此编写代码\n    return 0;\n}",
