...
export const paperData = {
    ...
    questions: [
        ...
        },
        {
            ...
            options: ["45", "28 int N, remainder; cout << \"请输入正整数:\"; cin >> N; cout << _________________; int a, b = (6, 28); b == a; a = b; cout << a << ' ' << b; int N, remainder; cin >> N; remainder = _____________; if(remainder == 0) printf(\"星期六", "3", "0"],
            answer: 3,
            score: 2,
            explanation: '答案依据官方答案。'
        },
        ...
        },
        {
            id: 10,
            type: 'single',
            question: "下⾯ C++ 代码执行后的输出是（ ）。 int i, j; for(i = 1; i < 12; i++) for(j = 1; j < i; j++) if(i * j % 2 == 1) break; cout << i * j; int i, cnt = 0; for(i = -99; i < 100; i += 2) cnt = 1 + cnt; cout << cnt; int i; for(i = 1; i < 10; i++){ if(i % 3 != 0){ printf(\"A#\"); continue; } else break; printf(\"0#\"); } if(i == 10) cout << \"1\";",
        ...
        },
        {
            ...
            options: ["", "", "", "int i,j; for(i = 0; i < 3; i++) for(j = 0; j < i; j++) printf(\"%d#%d-\", i, j); printf(\"END\"); int i,j; for(i = 0; i < 100; i++) if(______________________) cout << i << endl; int N, cnt; cout << \"请输入大于0的正整数：\"; cin >> N; c"],
            answer: 3,
            score: 2,
            explanation: '答案依据官方答案。'
        },
        ...
        },
        {
            id: 14,
            type: 'single',
            question: "下⾯C++代码实现输出如下图形，相关说法 的是（ ）。错误 N > 0 N /= 10 int N, N1, M1; cout << \"输入一个正整数：\"; cin >> N; N1 = N, M1 = N * N; bool Flag = true; while (N1 > 0){ if (N1 % 10 != M1 % 10){ Flag = false; break; } else{ N1 = N1 / 10, M1 = M1 / 10; } } if (Flag == true) printf(\"%d的平方是%d,是自守数\", N, N * N); else printf(\"%d的平方是%d,不是自守数\", N, N * N); 请输入层数: 10 int line_number, now_number; int i,row; cout << \"请输入行数: \"; cin >> line_number; now_number = 0; for (row = 1; row < line_number + 1; row++){ // L1 for (i = 0; i < row; i++){ // L2 cout << now_number; now_number += 1; if (now_number == 10) now_number = 0; // L3 题号 1 2 ",
            options: ["代码now_number = 0 移动到L1 和L2 标记的两行代码之间，效果维持不变", "代码now_number += 1 修改为now_number = 1 + now_number 效果维持不变", "将代码now_number == 10 调整为now_number > 9 效果维持不变", "将最后一行的cout << endl 修改为cout << \"\\n\" ，效果维持不变"],
            answer: 3,
            score: 2,
            explanation: '答案依据官方答案。'
        },
        ...
        },
        {
            id: 21,
            type: 'judge',
            question: "在下⾯的 C++ 代码中，因为continue 将被执行，因此不会有输出。（ ） } cout << endl; } int a = 10, b = 20; a == b; b == a; cout << (a, b); char a = '1'; a = 45.6; cout << a; int score; cout << \" 请输入学生成绩 : \"; cin >> score; if (score < 60) cout << \" 不及格 \"; else cout << \" 及格 \"; int i; for (i = 1; i < 10; i++) if (i % 2 == 0) continue; if(i == 10) cout << \"END\";",
            options: ['正确', '错误'],
            answer: 0,
            score: 2,
            explanation: '判断题答案待依据官方答案表人工复核。'
        ...
        },
        {
            id: 24,
            type: 'judge',
            question: "为实现如下效果，即 N 行 N 列字符。当输入是奇数时，中间列为* ，其他是 - ；当输入是偶数时，则中间两 列是* ，其他是 - 。字符阵列后的代码能实现其效果。（ ）",
            options: ['正确', '错误'],
            answer: 0,
            score: 2,
            explanation: '判断题答案待依据官方答案表人工复核。'
