    ...
    questions: [
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A N % 1000 / 100**：正确。答案依据官方答案。
            - **B N / 1000 % 100**：错误。应使用除法 / 而非取模 %，两者运算结果不同。
            - **C N / 1000 / 100**：错误。此说法不正确。答案依据官方答案。
            - **D N % 100 / 100**：错误。应使用除法 / 而非取模 %，两者运算结果不同。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A (N+6) / 7**：错误。此说法不正确。答案依据官方答案。
            - **B (N+6) // 7**：错误。应使用取模 % 而非除法 /，注意区分整除和取余。
            - **C N % 7**：错误。代码逻辑与正确答案不符。
            - **D (N+6) % 7**：正确。答案依据官方答案。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A A#A#**：正确。答案依据官方答案。
            - **B A#0#A#0**：错误。答案依据官方答案。
            - **C A#A#1**：错误。答案依据官方答案。
            - **D A#0#A#0#1**：错误。答案依据官方答案。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 0#0-1#0-2#0-2#1-END**：错误。此说法不正确。外层循环 i 从 0 到 2，内层循环 j 从 0 到 i。对应输出 i-j#。
            - **B 0#0-1#0-1#1-2#0-2#1-2#2-3#0-3#1-3#2-END**：错误。此说法不正确。外层循环 i 从 0 到 2，内层循环 j 从 0 到 i。对应输出 i-j#。
            - **C 0#0-1#0-1#1-2#0-2#1-2#2-END**：错误。此说法不正确。外层循环 i 从 0 到 2，内层循环 j 从 0 到 i。对应输出 i-j#。
            - **D 0-0#END\n1-0#1-1#END\n2-0#2-1#2-2#END**：正确。外层循环 i 从 0 到 2，内层循环 j 从 0 到 i。对应输出 i-j#。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A (i % 3 != 0) && (i % 5 == 2)**：错误。代码逻辑有误。!(i % 3) 表示 i 能被 3 整除，这与题目要求的“不能被 3 整除”相反。
            - **B (i % 3) && (i % 5 == 2)**：错误。代码逻辑有误。!(i % 3) 表示 i 能被 3 整除，这与题目要求的“不能被 3 整除”相反。
            - **C (i % 3) && !(i % 5 != 2)**：错误。混淆了比较 == 和赋值 =，条件判断应使用 ==。
            - **D !(i % 3) && (i % 5 == 2)**：正确。!(i % 3) 表示 i 能被 3 整除，这与题目要求的“不能被 3 整除”相反。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A N > 1；N = N / 10**：错误。此说法不正确。答案依据官方答案。
            - **B N > 1；N /= 10**：错误。代码逻辑与正确答案不符。
            - **C N == 0；N /= 10**：错误。混淆了赋值 = 和比较 ==，两者功能完全不同。
            - **D N > 0；N /= 10**：正确。答案依据官方答案。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 如果Flag在循环中不被改为false，则说明该数是⾃守数**：错误。此说法不正确。答案依据官方答案。
            - **B 代码 if (N1 % 10 != M1 % 10) 用于判断其个位数是否相等，...**：错误。代码逻辑与正确答案不符。
            - **C 代码 N1 = N1 / 10, M1 = M1 / 10 将个位数去掉**：错误。此说法不正确。答案依据官方答案。
            - **D 将 N1 > 0 改为 N > 0 效果相同**：正确。答案依据官方答案。
...
`,
      ...
      explanation: `
      **解析：**
      面积 ab/2 为整数等价于 ab 为偶数，也就是 a、b 不会同时为奇数。再结合 (a,b) 与 (b,a) 视为同一个三角形，只统计 a<=b 的方案即可。
      `,
      tags: ["编程题", "枚举", "数学"],
...
`,
      ...
      explanation: `
      **解析：**
      预先枚举所有满足 n=2^x+2^y 的数，去重后统计落在区间 [l,r] 内的个数即可。因为 2 的幂增长很快，指数范围很小。
      `,
      tags: ["编程题", "枚举", "集合"],
      template: "#include <bits/stdc++.h>\nusing namespace std;\n\nint main() {\n    ios::sync_with_stdio(false);\n    cin.tie(nullptr);\n\n    // 在此编写代码\n    return 0;\n}",
