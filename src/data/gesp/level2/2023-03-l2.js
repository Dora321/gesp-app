    ...
    questions: [
        {
            ...
            **解析：**
            ...
            - **A 硬盘**：错误。硬盘是磁存储介质，利用磁性记录数据，易受强磁场干扰。
            - **B U 盘**：错误。对不同存储介质的工作原理理解有误。
            - **C 内存**：错误。对不同存储介质的工作原理理解有误。
            - **D 光盘**：正确。光盘是采用激光热效应记录数据的光存储介质，不受磁场影响。硬盘、U盘和内存基于电子或磁性存储，易受强磁场干扰。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 顺序结构**：错误。流程图中出现了判断条件（是否按下空格键）并由此产生两个不同的执行路径，属于典型的分支结构。
            - **B 循环结构**：错误。对循环执行过程的分析有误。
            - **C 分支结构**：正确。流程图中出现了判断条件（是否按下空格键）并由此产生两个不同的执行路径，属于典型的分支结构。
            - **D 数据结构**：错误。流程图中出现了判断条件（是否按下空格键）并由此产生两个不同的执行路径，属于典型的分支结构。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A double 类型的变量占用内存的大小是浮动的**：正确。在 C++ 标准中，double 类型的大小通常固定为 8 字节，并非随数值大小浮动。
            - **B bool 类型的变量占用 1 字节内存**：错误。代码逻辑有误。在 C++ 标准中，double 类型的大小通常固定为 8 字节，并非随数值大小浮动。
            - **C int 类型变量的取值范围不是无限的**：错误。代码逻辑有误。在 C++ 标准中，double 类型的大小通常固定为 8 字节，并非随数值大小浮动。
            - **D char 类型的变量有 256 种取值**：错误。代码逻辑有误。在 C++ 标准中，double 类型的大小通常固定为 8 字节，并非随数值大小浮动。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 变量定义后，可以使用赋值语句改变它的值**：错误。对标识符命名规则理解有误。
            - **B 变量定义时，必须指定类型**：错误。对标识符命名规则理解有误。
            - **C 变量名必须为合法标识符**：错误。对标识符命名规则理解有误。
            - **D 合法标识符可以以数字开始**：正确。C++ 标识符必须由字母、数字或下划线组成，且第一个字符不能是数字。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A return**：错误。max 是标准库中的一个函数名，不是 C++ 语言定义的关键字。
            - **B max**：正确。max 是标准库中的一个函数名，不是 C++ 语言定义的关键字。
            - **C else**：错误。max 是标准库中的一个函数名，不是 C++ 语言定义的关键字。
            - **D case**：错误。max 是标准库中的一个函数名，不是 C++ 语言定义的关键字。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **B ==**：错误。混淆了赋值 = 和比较 ==，两者功能完全不同。
            - **C /=**：错误。在特定语境下，'=' 是赋值符。题目中选项 A 为 '=', 选项 D 可能是 '!' 或其他。根据提取结果，'=' 在某些分类中被选为非运算符（相对于算术/比…
            - **D ++**：错误。在特定语境下，'=' 是赋值符。题目中选项 A 为 '=', 选项 D 可能是 '!' 或其他。根据提取结果，'=' 在某些分类中被选为非运算符（相对于算术/比…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A "b = a+1;", "b = a+1;", "b = a++;", "b =...**：错误。'a' 是字符常量，常量的值不可更改，因此不能使用自增运算符 ++。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A (a+b+c+d)/4**：错误。此说法不正确。除以 4.0 可以强制将运算转换为浮点数计算，保留平均值的小数部分。
            - **B (a+b+c+d)%4**：错误。应使用除法 / 而非取模 %，两者运算结果不同。
            - **C (a+b+c+d)/4.0**：正确。除以 4.0 可以强制将运算转换为浮点数计算，保留平均值的小数部分。
            - **D (a+b+c+d)%4.0**：错误。应使用除法 / 而非取模 %，两者运算结果不同。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A a = a+1;**：错误。a+1; 只是一个表达式计算，结果未赋值回变量 a，因此 a 的值保持不变。
            - **B a+1;**：正确。a+1; 只是一个表达式计算，结果未赋值回变量 a，因此 a 的值保持不变。
            - **C a = 1+a;**：错误。a+1; 只是一个表达式计算，结果未赋值回变量 a，因此 a 的值保持不变。
            - **D ++a;**：错误。a+1; 只是一个表达式计算，结果未赋值回变量 a，因此 a 的值保持不变。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 3**：错误。可能只计算了一半的结果，如遗漏了某次循环或只取了部分数据。正确结果为 6。
            - **B 6**：正确。a -= 3 等价于 a = a-3，即 9-3 = 6。
            - **C 9**：错误。a -= 3 等价于 a = a-3，即 9-3 = 6。
            - **D 12**：错误。可能将结果翻倍计算了，如多循环了一轮或重复累加。正确结果为 6。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A (!a) || (!b)**：正确。!a 当 a 值为 0 时返回 true，|| 是逻辑或运算符。
            - **B (a == b == 0)**：错误。代码逻辑有误。!a 当 a 值为 0 时返回 true，|| 是逻辑或运算符。
            - **C (a == 0) && (b == 0)**：错误。应使用逻辑或 || 而非逻辑与 &&，两者语义不同。
            - **D (a == 0)-(b == 0) == 0**：错误。代码逻辑有误。!a 当 a 值为 0 时返回 true，|| 是逻辑或运算符。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A "a <= a <= z", "a-a <= z-a", "a <= a <= ...**：错误。在 C++ 中判断区间必须使用逻辑与 && 连接两个比较操作。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A a-=b**：错误。填入 a=b-a 后 a 变为 40。接着 b-=a 使得 b 变为 10。最后 a+=b 使得 a 变为 50。
            - **B a+=b**：错误。填入 a=b-a 后 a 变为 40。接着 b-=a 使得 b 变为 10。最后 a+=b 使得 a 变为 50。
            - **C a=b-a**：正确。填入 a=b-a 后 a 变为 40。接着 b-=a 使得 b 变为 10。最后 a+=b 使得 a 变为 50。
            - **D a=b**：错误。填入 a=b-a 后 a 变为 40。接着 b-=a 使得 b 变为 10。最后 a+=b 使得 a 变为 50。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A "ch < 5", "ch >= 5", "ch >= 4", "ch % 2 ...**：错误。在 '1' 到 '9' 之间，奇数（ASCII 码%2==1）有 '1','3','5','7','9' 共 5 个。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A false**：错误。由于循环条件 i <= n，当 i 到达 17 时，17%17==0 成立，isprime 变为 false。C++ 输…
            - **B true**：错误。由于循环条件 i <= n，当 i 到达 17 时，17%17==0 成立，isprime 变为 false。C++ 输…
            - **C 0**：正确。由于循环条件 i <= n，当 i 到达 17 时，17%17==0 成立，isprime 变为 false。C++ 输出布尔值 false 时显示 0。
            - **D 1**：错误。由于循环条件 i <= n，当 i 到达 17 时，17%17==0 成立，isprime 变为 false。C++ 输出布尔值 false 时显示 0。
    ...
    programmingQuestions: [
        {
            ...
            explanation: `
            **解析：**
            使用循环嵌套，外层控制行数，内层控制每行打印的字母。字母超出 'Z' 后需重置为 'A'。
            `,
            tags: ["循环", "字符处理"],
        ...
        },
        {
            ...
            explanation: `
            **解析：**
            通过双重循环枚举公鸡和母鸡的数量，剩余为小鸡，判断总钱数是否符合。注意用乘法避免浮点误差：z*x*i+z*y*j+k = z*n。
            `,
            tags: ["枚举", "模拟"],
            template: "#include <iostream>\nusing namespace std;\nint main() {\n    // 在此编写代码\n    return 0;\n}",
