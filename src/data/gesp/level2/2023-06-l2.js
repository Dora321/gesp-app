...
export const paperData = {
    ...
    questions: [
        ...
        **解析：**
        ...
        - **A if 条件判断 then 语句块**：错误。题意对应“条件不满足继续循环”。
        - **B if 条件判断 then 什么也不做 else 语句块**：错误。题意对应“条件不满足继续循环”。
        - **C while 条件判断 do 语句块**：错误。题意对应“条件不满足继续循环”。
        ...
        **解析：**
        ...
        - **A char 类型变量不能赋值给 int 类型变量**：错误。int * int 的结果类型仍为 int。
        - **B 两个 int 类型变量相乘，结果仍为 int 类型**：正确。int * int 的结果类型仍为 int。
        - **C 两个 int 相乘溢出时程序会报错崩溃**：错误。int * int 的结果类型仍为 int。
        ...
        **解析：**
        ...
        - **A if 的判断条件必须放在小括号中**：错误。多条语句必须用花括号，不是靠缩进。
        - **B for 语句中间循环条件可省略，表示恒为真**：错误。多条语句必须用花括号，不是靠缩进。
        - **C 循环体有多条语句时用缩进即可消除二义性**：正确。多条语句必须用花括号，不是靠缩进。
        - **D 运算符有优先级，不仅有先乘除后加减**：错误。多条语句必须用花括号，不是靠缩进。
        ...
        **解析：**
        ...
        - **D sqrt**：错误。double 是关键字，其余是常见标识符/函数名。
        ...
        **解析：**
        ...
        - **A for (int a = 0; a; a++) ;**：错误。bool 自增后会保持 true，条件始终成立。
        - **B for (bool b = false; b <= true; b++) ;**：正确。bool 自增后会保持 true，条件始终成立。
        - **C for (char c = \A\; c < \z\; c++) ;**：错误。bool 自增后会保持 true，条件始终成立。
        - **D for (double d = 0.0; d < 10.0; d += 0.00...**：错误。bool 自增后会保持 true，条件始终成立。
        ...
        **解析：**
        ...
        - **A E**：错误。表达式提升为 int，输出数值 69。
        - **B C+2**：错误。表达式提升为 int，输出数值 69。
        - **C C2**：错误。表达式提升为 int，输出数值 69。
        ...
        **解析：**
        ...
        - **A (a+b)**：错误。需先按 ASCII 计算并转换回 char。
        - **B (a+b-\0\)**：错误。对字符串或字符的处理方式理解有误。
        - **C (char)(a+b)**：错误。需先按 ASCII 计算并转换回 char。
        ...
        **解析：**
        ...
        - **A A**：错误。根据原卷答案，第15题为 C。
        - **B B**：错误。根据原卷答案，第15题为 C。
        - **C C**：正确。根据原卷答案，第15题为 C。
        - **D D**：错误。根据原卷答案，第15题为 C。
...
`,
      ...
      explanation: `
      **解析：**
      在区间 [A,B] 中枚举每个整数，判断它是否是素数；若是素数就把答案加一。
      `,
      tags: ["编程题", "枚举", "素数"],
...
`,
      ...
      explanation: `
      **解析：**
      设整数有 len 位，拆出它的每一位并累加 digit^len。若结果恰好等于原数，就是自幂数。
      `,
      tags: ["编程题", "模拟", "数位分解"],
      template: "#include <bits/stdc++.h>\nusing namespace std;\n\nint main() {\n    ios::sync_with_stdio(false);\n    cin.tie(nullptr);\n\n    // 在此编写代码\n    return 0;\n}",
