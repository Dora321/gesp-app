    ...
    questions: [
        {
            ...
            **解析：**
            ...
            - **A while (p != nullptr) { cout << p->data <...**：错误。该代码逻辑与题目要求不符，请逐步推演。
            - **B while (p->next != nullptr) { cout << p->...**：错误。该代码逻辑与题目要求不符，请逐步推演。
            - **C do { cout << p->data << \\**：正确答案。
            - **D for (; p; p = p->next) { cout << p->data...**：错误。该代码逻辑与题目要求不符，请逐步推演。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 双链表删除指定结点通常是 $O(N)$，单链表也是 $O(N)$。**：错误。对链表结构的理解有误，请重新分析节点的指针指向与边界情况。
            - **B 双链表删除指定结点通常是 $O(1)$，单链表也是 $O(1)$。**：错误。对链表结构的理解有误，请重新分析节点的指针指向与边界情况。
            - **C 双链表删除指定结点通常是 $O(1)$，单链表若需先找前驱通常是 $O(N)$。**：正确答案。
            - **D 双链表删除指定结点通常是 $O(N)$，单链表若已知前驱则是 $O(1)$。**：错误。对链表结构的理解有误，请重新分析节点的指针指向与边界情况。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A gcd1() 实现为递归方式。**：错误。递归/递推的终止条件或状态传递有误，请检查递归出口和参数变化。
            - **B gcd2() 实现为迭代方式。**：错误。数论计算有误，请重新验算质因数分解或 gcd 过程。
            - **C 当输入较大时，gcd1() 会多次递归调用自身，需要额外的栈空间。**：错误。递归/递推的终止条件或状态传递有误，请检查递归出口和参数变化。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 快速排序是稳定排序**：错误。不同排序算法的稳定性或复杂度理解有误，请对照正确解析。
            - **B 归并排序通常是稳定的**：正确答案。
            - **C 插入排序是不稳定排序**：错误。不同排序算法的稳定性或复杂度理解有误，请对照正确解析。
            - **D 冒泡排序不是原地排序**：错误。不同排序算法的稳定性或复杂度理解有误，请对照正确解析。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 归并排序的平均时间复杂度是 $O(n log n)$。**：错误。不同排序算法的稳定性或复杂度理解有误，请对照正确解析。
            - **B 归并排序通常需要 $O(N)$ 的额外空间。**：错误。不同排序算法的稳定性或复杂度理解有误，请对照正确解析。
            - **C 归并排序在最坏情况下的时间复杂度是 $O(N^2)$。**：正确答案。
            - **D 归并排序适合大规模数据。**：错误。不同排序算法的稳定性或复杂度理解有误，请对照正确解析。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **B 两种实现方式的空间复杂度相同，都是 $O(1)$。**：错误。复杂度分析有误，请重新估算最坏情况时间复杂度。
            - **C 两种实现方式的空间复杂度相同，都是 $O(N)$。**：错误。复杂度分析有误，请重新估算最坏情况时间复杂度。
            - **D factorial1() 的时间复杂度为 $O(1)$，factorial2()...**：错误。复杂度分析有误，请重新估算最坏情况时间复杂度。
