...
const programmingQuestions = [
    {
      ...
      score: 25,
      description: '若 1 / a 可化为一个有限的、不循环的小数，则称 a 为终止数。请你求出 L 到 R 中终止数的数量。\n\n数据范围：1 ≤ L ≤ R ≤ 10^6。',
      ...
      explanation: `**解析：**
      分数 \`1 / a\` 能写成有限小数，当且仅当约分后的分母只含质因数 2 和 5。由于分子固定为 1，\`a\` 本身必须形如 \`2^x × 5^y\`。

      枚举 \`[L, R]\` 中的每个整数 \`a\`，反复除去所有因子 2 和 5。如果最后剩下 1，说明它没有其他质因子，是终止数；否则不是。

      例如 \`[2, 11]\` 中：
      - \`2、4、5、8、10\` 只含质因数 2 和 5；
      - \`3、6、7、9、11\` 至少含有其他质因数。

      因此样例答案为 5。每个数最多进行 \`O(log R)\` 次除法，总时间复杂度为 \`O((R-L+1) log R)\`，额外空间复杂度为 \`O(1)\`。
