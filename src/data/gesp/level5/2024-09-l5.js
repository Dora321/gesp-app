    ...
    questions: [
        {
            ...
            **解析：**
            ...
            - **A 数组大小固定，链表大小可动态调整。**：不是本题答案。
            - **B 数组支持随机访问，链表只能顺序访问。**：不是本题答案。
            - **C 存储相同数目的整数，数组比链表所需的内存多。**：正确答案。
            - **D 数组插入和删除元素效率低，链表插入和删除元素效率高。**：不是本题答案。
            **考点：** 链表
            `,
            tags: ["客观题", "单选题", "GESP5级"]
        },
...
**考点：** 归并排序、递归树、函数调用次数`,
        ...
        },
        {
            id: 11,
            type: "single",
            question: `现在有 n 个人要过河，每只船最多载 2 人，船的承重为 100kg。下列代码中，数组 weight 中保存有 n 个人的体重（单位为 kg），已经按从小到大排好序，代码输出过河所需要的船的数目，采用的思想为（　）。
...
代码用两个指针从体重数组的两端往中间靠：\`i\` 指向最轻的人，\`j\` 指向最重的人。

每一轮都做同一个**局部最优**决策：让当前**最重**的人上船，再看当前**最轻**的人能否与他同乘。
- 若 \`weight[i] + weight[j] <= 100\`，两人合乘一条船，\`i++\`（最轻的人也走了）。
- 否则最重的人只能独乘一条船。

无论哪种情况 \`j--\` 且 \`count++\`，即最重的人每轮必定离开。这种「每一步都取眼前最优、不回头重算」的策略正是**贪心**。

**逐项分析**：
- **A 枚举算法**：枚举需要穷举所有可能的配对方案再择优，这里没有穷举，每步的选择是直接确定的。
- **B 贪心算法**：正确。
- **C 迭代算法**：「迭代」只是描述用循环实现，不是解题**思想**层面的分类；按这个说法几乎所有循环代码都算迭代，无法区分本题特征。
- **D 递归算法**：代码中没有函数自我调用。

**为什么这样贪心是对的**：最重的人无论如何都要占一条船，那就让他尽量捎上当前最轻的人——若最轻的都配不上他，其他人更配不上。

**考点**：双指针贪心（最轻配最重）与算法思想的辨析`,
            tags: ["客观题", "单选题", "GESP5级"]
        },
        {
            id: 12,
