    ...
    questions: [
        {
            ...
            **解析：**
            ...
            - **A 12 字节**：错误。此说法有误，Base64 每 3 字节映射为 4 字节。10 字节可分为 3 组完整的 3 字节和 1 个剩余字节，因此编码后长度为…
            - **B 13 字节**：错误。此说法有误，Base64 每 3 字节映射为 4 字节。10 字节可分为 3 组完整的 3 字节和 1 个剩余字节，因此编码后长度为…
            - **C 14 字节**：错误。此说法有误，Base64 每 3 字节映射为 4 字节。10 字节可分为 3 组完整的 3 字节和 1 个剩余字节，因此编码后长度为…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 0xC0 0x80**：错误。此说法有误，B 符合 4 字节 UTF-8 的前缀与后续字节格式；A 属于过长编码，C 不能以续字节开头，D 不属于合法 UTF-8…
            - **B 0xF0 0x90 0x80 0x80**：正确。B 符合 4 字节 UTF-8 的前缀与后续字节格式；A 属于过长编码，C 不能以续字节开头，D 不属于合法 UTF-8 起始字节。
            - **C 0x80 0x80 0x80**：错误。此说法有误，B 符合 4 字节 UTF-8 的前缀与后续字节格式；A 属于过长编码，C 不能以续字节开头，D 不属于合法 UTF-8…
            - **D 0xFF 0xFE 0xFD**：错误。此说法有误，B 符合 4 字节 UTF-8 的前缀与后续字节格式；A 属于过长编码，C 不能以续字节开头，D 不属于合法 UTF-8…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **B 11111010**：错误。正确结果为 10000101，此选项 11111010 计算有误
            - **C 11111011**：错误。正确结果为 10000101，此选项 11111011 计算有误
            - **D 00000101**：错误。正确结果为 10000101，此选项 00000101 计算有误
        ...
        },
        {
            ...
            **解析：**
            ...
            - **B 1101110.1001110001**：错误。正确结果为 1101111.0001110001，此选项 1101110.1001110001 不正确
            - **C 1101111.1001110001**：错误。正确结果为 1101111.0001110001，此选项 1101111.1001110001 不正确
            - **D 1101111.0011110001**：错误。正确结果为 1101111.0001110001，此选项 1101111.0011110001 不正确
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 提高浮点数的精度**：错误。此说法有误，补码的核心价值是统一加减法硬件实现，把减法转化为加法处理。
            - **B 简化整数的加减法运算**：正确。补码的核心价值是统一加减法硬件实现，把减法转化为加法处理。
            - **C 增加整数的表示范围**：错误。此说法有误，补码的核心价值是统一加减法硬件实现，把减法转化为加法处理。
            - **D 优化内存分配**：错误。此说法有误，补码的核心价值是统一加减法硬件实现，把减法转化为加法处理。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **B -127 到 128**：错误。此说法有误，8 位补码有 1 位符号位，范围是 -2^7 到 2^7-1，即 -128 到 127。
            - **C -256 到 255**：错误。此说法有误，8 位补码有 1 位符号位，范围是 -2^7 到 2^7-1，即 -128 到 127。
            - **D 0 到 255**：错误。此说法有误，8 位补码有 1 位符号位，范围是 -2^7 到 2^7-1，即 -128 到 127。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A -5**：错误。正确结果为 4294967291，此选项 -5 计算有误
            - **B 5**：错误。正确结果为 4294967291，此选项 5 计算有误
            - **C 4294967291**：正确。把 -5 赋给 32 位 unsigned int 会按补码位模式解释为 2^32-5，即 4294967291。
            - **D 编译错误**：错误。此说法有误，把 -5 赋给 32 位 unsigned int 会按补码位模式解释为 2^32-5，即 4294967291。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **D 不能正确执行**：错误。此说法有误，hex 会把后续整数按十六进制输出。
