    ...
    questions: [
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 0xC0 0x80**：错误。B 符合 4 字节 UTF-8 的前缀与后续字节格式；A 属于过长编码，C 不能以续字节开头，D 不属于合法 UTF-8…
            - **B 0xF0 0x90 0x80 0x80**：正确。B 符合 4 字节 UTF-8 的前缀与后续字节格式；A 属于过长编码，C 不能以续字节开头，D 不属于合法 UTF-8 起始字节。
            - **C 0x80 0x80 0x80**：错误。B 符合 4 字节 UTF-8 的前缀与后续字节格式；A 属于过长编码，C 不能以续字节开头，D 不属于合法 UTF-8…
            - **D 0xFF 0xFE 0xFD**：错误。B 符合 4 字节 UTF-8 的前缀与后续字节格式；A 属于过长编码，C 不能以续字节开头，D 不属于合法 UTF-8…
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A 提高浮点数的精度**：错误。补码的核心价值是统一加减法硬件实现，把减法转化为加法处理。
            - **B 简化整数的加减法运算**：正确。补码的核心价值是统一加减法硬件实现，把减法转化为加法处理。
            - **C 增加整数的表示范围**：错误。补码的核心价值是统一加减法硬件实现，把减法转化为加法处理。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **B -127 到 128**：错误。8 位补码有 1 位符号位，范围是 -2^7 到 2^7-1，即 -128 到 127。
            - **C -256 到 255**：错误。8 位补码有 1 位符号位，范围是 -2^7 到 2^7-1，即 -128 到 127。
            - **D 0 到 255**：错误。8 位补码有 1 位符号位，范围是 -2^7 到 2^7-1，即 -128 到 127。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A cout << oct << decimal;**：错误。hex 会把后续整数按十六进制输出。
            - **B cout << decimal << decimal;**：错误。hex 会把后续整数按十六进制输出。
            - **C cout << hex << decimal;**：正确。hex 会把后续整数按十六进制输出。
            - **D 不能正确执行**：错误。hex 会把后续整数按十六进制输出。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A if (arrmaxIndex > arri)**：错误。枚举最大值下标时，应比较当前位置元素和当前最大值元素，即 arr[i] > arr[maxIndex]。
            - **B if (arri-1 > arrmaxIndex)**：错误。枚举最大值下标时，应比较当前位置元素和当前最大值元素，即 arr[i] > arr[maxIndex]。
            - **C if (arri+1 > arrmaxIndex)**：错误。枚举最大值下标时，应比较当前位置元素和当前最大值元素，即 arr[i] > arr[maxIndex]。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A str.replace(0, 5, C++);**：错误。把下标 5 开始的 5 个字符 World 替换为 C++，结果就是 HelloC++。
            - **B str.replace(5, 5, C++);**：正确。把下标 5 开始的 5 个字符 World 替换为 C++，结果就是 HelloC++。
            - **C str.replace(1, 5, C++);**：错误。把下标 5 开始的 5 个字符 World 替换为 C++，结果就是 HelloC++。
            - **D str.replace(4, 5, C++);**：错误。把下标 5 开始的 5 个字符 World 替换为 C++，结果就是 HelloC++。
        ...
        },
        {
            ...
            **解析：**
            ...
            - **A str.insert(4, World);\\ncout << str.subst...**：错误。在下标 5 处插入 World 后，再从下标 5 开始截取长度 5，即可得到 World。
            - **B cout << str.substr(5, 5);**：错误。在下标 5 处插入 World 后，再从下标 5 开始截取长度 5，即可得到 World。
            - **C str.insert(World);\\ncout << str.substr(5...**：错误。在下标 5 处插入 World 后，再从下标 5 开始截取长度 5，即可得到 World。
