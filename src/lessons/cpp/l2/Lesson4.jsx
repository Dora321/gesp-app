...
import CppLessonShell, { MasteryCheck, PredictCheck, TransferCheck } from '../CppLessonShell';
...
export default function CppL2Lesson4() {
    return (
        <CppLessonShell
            lessonNumber={4}
            lessonTitle="神奇的开关 (Switch)"
            lessonSubtitle="case / break / default 三件套"
            accent="indigo"
            sections={sections}
            previousPath="/lesson/2/3"
            nextPath="/lesson/2/5"
            topSupport={<CppL2LessonSupport lessonId={4} />}
            bottomSupport={<CppL2LessonSupport lessonId={4} placement="bottom" />}
            hero={{
                title: '选项很多时，别让 if 排成长队',
                description: '游戏菜单、星期判断、等级选项这类“固定编号匹配”场景，用 switch 更清楚。今天要把 case、break、default 这三个关键件装稳。',
            }}
            goals={['读懂 switch-case 执行顺序', '知道 break 防止穿透', '会用 default 处理异常输入']}
            prerequisites={['会用 if / else 做条件判断', '理解整数、字符这类离散值', '会用 cin 读入一个值']}
            childrenBySection={{
                1: <MenuSimulator />,
                2: (
